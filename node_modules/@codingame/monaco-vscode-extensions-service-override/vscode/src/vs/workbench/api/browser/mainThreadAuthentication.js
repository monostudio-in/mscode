...
let MainThreadAuthentication = class MainThreadAuthentication extends Disposable {
    ...
    $showContinueNotification(message) {
        const yes = ( localize(2523, "Yes"));
        const no = ( localize(2524, "No"));
        const deferredPromise = ( new DeferredPromise());
        let result = false;
    ...
    async loginPrompt(provider, extensionName, recreatingSession, options) {
        ...
        if (options?.learnMore) {
            buttons.push({
                label: ( localize(2528, "Learn more")),
                run: async () => {
    ...
    async continueWithIncorrectAccountPrompt(chosenAccountLabel, requestedAccountLabel) {
        const result = await this.dialogService.prompt({
            message: ( localize(2529, "Incorrect account detected")),
            detail: ( localize(
                2530,
            ...
            cancelButton: true,
            buttons: [{
                label: ( localize(2531, "Keep {0}", chosenAccountLabel)),
                run: () => chosenAccountLabel
            }, {
                label: ( localize(2532, "Login with {0}", requestedAccountLabel)),
    ...
    async $showDeviceCodeModal(userCode, verificationUri) {
        const {
            ...
            type: Severity.Info,
            message: ( localize(2533, "Device Code Authentication")),
            detail: ( localize(
                2534,
            ...
            buttons: [{
                label: ( localize(2535, "Copy & Continue")),
                run: () => true
            }],
            cancelButton: true
        });
        if (result) {
            try {
                ...
                this.notificationService.error(( localize(2536, "Failed to open {0}", verificationUri)));
            }
        }
        return false;
    }
    async $promptForClientRegistration(authorizationServerUrl) {
        ...
        const result = await this.dialogService.prompt({
            type: Severity.Info,
            message: ( localize(2537, "Dynamic Client Registration not supported")),
            detail: ( localize(
                2538,
            ...
            buttons: [{
                label: ( localize(2539, "Copy URIs & Proceed")),
                run: async () => {
                    try {
                        await this.clipboardService.writeText(redirectUrls);
                    } catch (error) {
                        this.notificationService.error(( localize(2540, "Failed to copy redirect URIs to clipboard.")));
                    }
                    return true;
                }
            }],
            cancelButton: {
                label: ( localize(2541, "Cancel")),
                run: () => false
            }
        });
        if (!result) {
        ...
        const sharedTitle = ( localize(2542, "Add Client Registration Details"));
        const clientId = await this.quickInputService.input({
            ...
            placeHolder: ( localize(2544, "OAuth client ID (azye39d...)")),
            ignoreFocusLost: true,
            validateInput: async value => {
                if (!value || value.trim().length === 0) {
                    return localize(2545, "Client ID is required");
                }
                return undefined;
            }
        });
        ...
        const clientSecret = await this.quickInputService.input({
            ...
            placeHolder: ( localize(2547, "OAuth client secret (wer32o50f...) or leave it blank")),
            password: true,
            ignoreFocusLost: true
        });
        return {
