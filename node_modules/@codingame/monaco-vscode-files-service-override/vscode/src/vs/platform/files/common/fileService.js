...
let FileService = class FileService extends Disposable {
    ...
    registerProvider(scheme, provider) {
        ...
        providerDisposables.add(
            provider.onDidChangeCapabilities(() => this._onDidChangeFileSystemProviderCapabilities.fire({
                provider,
                scheme
            }))
        );
    ...
    async doResolveFile(resource, options) {
        ...
        return this.toFileStat(
            ...
            (stat, siblings) => {
                ...
                if (trie.get(stat.resource) || trie.findSuperstr(stat.resource.with({
                    query: null,
                    fragment: null
                } ))) {
                    return true;
            ...
            try {
                ...
                const resolvedEntries = await Promises.settled(( entries.map(async ([name, type]) => {
                    try {
                        ...
                        const childStat = resolveMetadata ? await provider.stat(childResource) : {
                            type
                        };
    ...
    async writeFile(resource, bufferOrReadableOrStream, options) {
        ...
        try {
            ...
            if (
            !hasOpenReadWriteCloseCapability(provider) || (hasReadWriteCapability(provider) && bufferOrReadableOrStreamOrBufferedStream instanceof VSBuffer) || (hasReadWriteCapability(provider) && hasFileAtomicWriteCapability(provider) && writeFileOptions?.atomic)) {
    ...
    async validateWriteFile(provider, resource, bufferOrReadableOrStream, options) {
        ...
        if (typeof options?.mtime === "number" && typeof options.etag === "string" && options.etag !== ETAG_DISABLED && typeof stat.mtime === "number" && typeof stat.size === "number" && options.mtime < stat.mtime && options.etag !== etag({
            ...
            throw ( new FileOperationError(( localize(1965, "File Modified Since")), FileOperationResult.FILE_MODIFIED_SINCE, options));
        }
        return {
            stat,
            buffer
    ...
    async doReadFileStream(provider, resource, options, token) {
        ...
        try {
            ...
            if (
            (readFileOptions?.atomic && hasFileAtomicReadCapability(provider)) || !(hasOpenReadWriteCloseCapability(provider) || hasFileReadStreamCapability(provider)) || (hasReadWriteCapability(provider) && readFileOptions?.preferUnbuffered)) {
    ...
    async validateReadFile(resource, options) {
        ...
        if (typeof options?.etag === "string" && options.etag !== ETAG_DISABLED && options.etag === stat.etag) {
            throw ( new NotModifiedSinceFileOperationError(( localize(1968, "File not modified since")), stat, options));
        }
    ...
    async move(source, target, overwrite) {
        ...
        const fileStat = await this.resolve(target, {
            resolveMetadata: true
        });
    ...
    async cloneFile(source, target) {
        ...
        if (sourceProvider === targetProvider && hasFileFolderCopyCapability(sourceProvider)) {
            return this.writeQueue.queueFor(source, () => sourceProvider.copy(source, target, {
                overwrite: true
    ...
    async doPipeBufferedQueued(sourceProvider, source, targetProvider, target) {
        ...
        try {
            sourceHandle = await sourceProvider.open(source, {
                create: false
            });
...
FileService = FileService_1 = ( __decorate([( __param(0, ILogService))], FileService));
