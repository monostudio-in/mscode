...
async function doReadFileIntoStream(provider, resource, target, transformer, options, token) {
    ...
    try {
        ...
        let buffer = VSBuffer.alloc(Math.min(
            options.bufferSize,
        ...
        do {
            ...
            if (posInBuffer === buffer.byteLength) {
                ...
                buffer = VSBuffer.alloc(Math.min(
                    options.bufferSize,
                    typeof allowedRemainingBytes === "number" ? allowedRemainingBytes : options.bufferSize
                ));
                posInBuffer = 0;
...
function throwIfTooLarge(totalBytesRead, options) {
    if (typeof options?.limits?.size === "number" && totalBytesRead > options.limits.size) {
        throw createFileSystemProviderError(( localize(1986, "File is too large to open")), FileSystemProviderErrorCode.FileTooLarge);
    }
    return true;
}
