...
let AbstractTextFileService = class AbstractTextFileService extends Disposable {
    ...
    static {
        this.TEXTFILE_SAVE_CREATE_SOURCE = SaveSourceRegistry.registerSource("textFileCreate.source", ( localize(16604, "File Created")));
    }
    static {
        this.TEXTFILE_SAVE_REPLACE_SOURCE = SaveSourceRegistry.registerSource("textFileOverwrite.source", ( localize(16605, "File Replaced")));
    }
    constructor(
        fileService,
    ...
    provideDecorations() {
        const provider = this._register(new (class extends Disposable {
            constructor(files) {
                super();
                this.files = files;
                this.label = ( localize(16606, "Text File Model Decorations"));
                this._onDidChange = this._register(( new Emitter()));
                this.onDidChange = this._onDidChange.event;
                this.registerListeners();
            }
            registerListeners() {
                this._register(this.files.onDidResolve((
                    {
                        model
                    }
                ) => {
            ...
            provideDecorations(uri) {
                ...
                if (isReadonly && isOrphaned) {
                    return {
                        ...
                        strikethrough: true,
                        tooltip: ( localize(16607, "Deleted, Read-only"))
                    };
                }
                else if (isReadonly) {
                    return {
                        letter: Codicon.lockSmall,
                        tooltip: ( localize(16608, "Read-only"))
                    };
                }
                else if (isOrphaned) {
                    return {
                        color: listErrorForeground,
                        strikethrough: true,
                        tooltip: ( localize(16609, "Deleted"))
                    };
                }
                return undefined;
            }
    ...
    async doRead(resource, options) {
        ...
        try {
            ...
            if (error.decodeStreamErrorKind === DecodeStreamErrorKind.STREAM_IS_BINARY) {
                throw ( new TextFileOperationError(( localize(16610, "File seems to be binary and cannot be opened as text")), TextFileOperationResult.FILE_IS_BINARY, options));
            }
            else {
                throw error;
            }
    ...
    async getEncodedReadable(resource, value, options) {
        const {
            encoding,
            addBOM
        } = await this.encoding.getWriteEncoding(resource, options);
        ...
        return toEncodeReadable(snapshot, encoding, {
            addBOM
        });
    }
    ...
    async validateDetectedEncoding(resource, detectedEncoding, options) {
        const {
            encoding
        } = await this.encoding.getPreferredReadEncoding(resource, options, detectedEncoding);
        return encoding;
    }
    ...
    async doSaveAsTextFile(sourceModel, source, target, options) {
        ...
        let write;
        if (sourceModel instanceof UntitledTextEditorModel && sourceModel.hasAssociatedFilePath && targetExists && this.uriIdentityService.extUri.isEqual(target, toLocalResource(
    ...
    async confirmOverwrite(resource) {
        const {
            ...
            primaryButton: ( localize(16613, "&&Replace"))
        });
        return confirmed;
    }
    async confirmMakeWriteable(resource) {
        const {
            ...
            detail: ( localize(16615, "Paths can be configured as read-only via settings.")),
            primaryButton: ( localize(16616, "&&Save Anyway"))
        });
        return confirmed;
    }
...
AbstractTextFileService = AbstractTextFileService_1 = ( __decorate([( __param(0, IFileService)), ( __param(1, IUntitledTextEditorService)), ( __param(2, ILifecycleService)), ( __param(3, IInstantiationService)), ( __param(4, IModelService)), ( __param(5, IWorkbenchEnvironmentService)), ( __param(6, IDialogService)), ( __param(7, IFileDialogService)), ( __param(8, ITextResourceConfigurationService)), ( __param(9, IFilesConfigurationService)), ( __param(10, ICodeEditorService)), ( __param(11, IPathService)), ( __param(12, IWorkingCopyFileService)), ( __param(13, IUriIdentityService)), ( __param(14, ILanguageService)), ( __param(15, ILogService)), ( __param(16, IElevatedFileService)), ( __param(17, IDecorationsService))], AbstractTextFileService));
let EncodingOracle = class EncodingOracle extends Disposable {
    ...
    registerListeners() {
        this._register(
            this.contextService.onDidChangeWorkspaceFolders(() => this.encodingOverrides = this.getDefaultEncodingOverrides())
        );
    }
    ...
    async getWriteEncoding(resource, options) {
        const {
            encoding,
            hasBOM
        } = await this.getPreferredWriteEncoding(resource, options ? options.encoding : undefined);
        return {
            encoding,
            addBOM: hasBOM
        };
...
EncodingOracle = ( __decorate([( __param(0, ITextResourceConfigurationService)), ( __param(1, IWorkbenchEnvironmentService)), ( __param(2, IWorkspaceContextService)), ( __param(3, IUriIdentityService))], EncodingOracle));
