...
let AbstractCommandsQuickAccessProvider = class AbstractCommandsQuickAccessProvider extends PickerQuickAccessProvider {
    ...
    async _getPicks(filter, _disposables, token, runOptions) {
        ...
        const runTfidf = createSingleCallFunction(() => {
            ...
            return normalizeTfIdfScores(result).filter(
                score => score.score > AbstractCommandsQuickAccessProvider_1.TFIDF_THRESHOLD
        ...
        for (let i = 0; i < filteredCommandPicks.length; i++) {
            ...
            if (i === 0 && isInHistory) {
                commandPicks.push({
                    type: "separator",
                    label: ( localize(2055, "recently used"))
                });
            ...
            if (addSuggestedSeparator && commandPick.tfIdfScore !== undefined) {
                commandPicks.push({
                    type: "separator",
                    label: ( localize(2056, "similar commands"))
                });
            ...
            if (addCommonlyUsedSeparator && commandPick.tfIdfScore === undefined && !isInHistory && this.options.suggestedCommandIds?.has(commandPick.commandId)) {
                commandPicks.push({
                    type: "separator",
                    label: ( localize(2057, "commonly used"))
                });
                addOtherSeparator = true;
            ...
            if (addOtherSeparator && commandPick.tfIdfScore === undefined && !isInHistory && !this.options.suggestedCommandIds?.has(commandPick.commandId)) {
                commandPicks.push({
                    type: "separator",
                    label: ( localize(2058, "other commands"))
                });
    ...
    toCommandPick(commandPick, runOptions, isRecentlyUsed = false) {
        ...
        const ariaLabel = keybinding ? ( localize(2059, "{0}, {1}", commandPick.label, keybinding.getAriaLabel())) : commandPick.label;
        const existingButtons = commandPick.buttons || [];
        const buttons = isRecentlyUsed ? [...existingButtons, {
            iconClass: ThemeIcon.asClassName(Codicon.close),
            tooltip: ( localize(2060, "Remove from Recently Used"))
        }] : commandPick.buttons;
        return {
            ...commandPick,
            tooltip,
            ...
            accept: async () => {
                ...
                try {
                    ...
                    if (!isCancellationError(error)) {
                        this.dialogService.error(( localize(2061, "Command '{0}' resulted in an error", commandPick.label)), toErrorMessage(error));
                    }
                }
            },
