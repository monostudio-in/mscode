...
let InlineCompletionsSource = class InlineCompletionsSource extends Disposable {
    ...
    constructor(
        ...
        this._state = observableReducerSettable(this, {
            ...
            changeTracker: recordChangesLazy(() => ({
        ...
        this._structuredFetchLogger = this._register(
            this._instantiationService.createInstance(StructuredLogger.cast(), "editor.inlineSuggest.logFetch.commandId")
        );
    ...
    fetch(
        ...
        const promise = (async () => {
            ...
            try {
                ...
                runWhenCancelled(source.token, () => providerResult.cancelAndDispose({
                ...
                for await (const list of providerResult.lists) {
                    ...
                    list.addRef();
                    store.add(
                        toDisposable(() => list.removeRef(list.inlineSuggestionsData.length === 0 ? {
                            kind: "empty"
                        } : {
                            kind: "notTaken"
                        }))
...
InlineCompletionsSource = InlineCompletionsSource_1 = ( __decorate([( __param(4, ILanguageConfigurationService)), ( __param(5, ILogService)), ( __param(6, IConfigurationService)), ( __param(7, IInstantiationService)), ( __param(8, IContextKeyService)), ( __param(9, ITextModelService))], InlineCompletionsSource));
class UpdateRequest {
    ...
    satisfies(other) {
        return this.position.equals(other.position) && equalsIfDefined(
            ...
            thisEqualsC()
        ) && (other.context.triggerKind === InlineCompletionTriggerKind.Automatic || this.context.triggerKind === InlineCompletionTriggerKind.Explicit) && this.versionId === other.versionId && isSubset(other.providers, this.providers);
    }
...
class InlineCompletionsState extends Disposable {
    ...
    createStateWithAppliedResults(
        ...
        const preferInlineCompletions =
        itemToPreserve ?
        !itemToPreserve.isInlineEdit : ( updatedSuggestions.some(i => !i.isInlineEdit && i.isVisible(textModel, cursorPosition)));
        let updatedItems = [];
