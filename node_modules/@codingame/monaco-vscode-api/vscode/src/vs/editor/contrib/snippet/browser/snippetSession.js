        ...
        if (this._placeholderGroupsIdx >= 0) {
            ...
            for (const placeholder of this._placeholderGroups[this._placeholderGroupsIdx]) {
                if (placeholder.transform) {
                    ...
                    operations.push(
                        EditOperation.replace(range, transformedValueLines.join(this._editor.getModel().getEOL()))
                    );
                }
            }
...
let SnippetSession = SnippetSession_1 = class SnippetSession {
    ...
    static createEditsAndSnippetsFromEdits(
        ...
        const resolver = ( new CompositeSnippetVariableResolver([editor.invokeWithinContext(accessor => ( new ModelBasedVariableResolver(accessor.get(ILabelService), model))), ( new ClipboardBasedVariableResolver(
            () => clipboardText,
            0,
            editor.getSelections().length,
            editor.getOption(EditorOption.multiCursorPaste) === "spread"
        )), ( new SelectionBasedVariableResolver(model, editor.getSelection(), 0, overtypingCapturer)), ( new CommentBasedVariableResolver(model, editor.getSelection(), languageConfigurationService)), ( new TimeBasedVariableResolver()), ( new WorkspaceBasedVariableResolver(
