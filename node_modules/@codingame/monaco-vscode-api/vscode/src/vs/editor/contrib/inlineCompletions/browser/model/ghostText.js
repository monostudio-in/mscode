...
class GhostText {
    ...
    render(documentText, debug = false) {
        return ( new TextEdit([...( this.parts.map(p => ( new TextReplacement(Range.fromPositions(( new Position(this.lineNumber, p.column))), debug ? `[${( p.lines.map(line => line.line)).join("\n")}]` : ( p.lines.map(line => line.line)).join("\n")))))])).applyToString(documentText);
    }
...
class GhostTextReplacement {
    ...
    render(documentText, debug = false) {
        ...
        if (debug) {
            return ( new TextEdit([( new TextReplacement(Range.fromPositions(replaceRange.getStartPosition()), "(")), ( new TextReplacement(
