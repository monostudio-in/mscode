...
class GuidesTextModelPart extends TextModelPart {
    ...
    getLinesBracketGuides(startLineNumber, endLineNumber, activePosition, options) {
        ...
        if (activePosition && bracketPairs.length > 0) {
            ...
            startLineNumber <= activePosition.lineNumber && activePosition.lineNumber <= endLineNumber ? bracketPairs : this.textModel.bracketPairs.getBracketPairsInRange(Range.fromPositions(activePosition)).toArray()).filter(bp => Range.strictContainsPosition(bp.range, activePosition));
            activeBracketPairRange = findLast(
                bracketsContainingActivePosition,
                i => includeSingleLinePairs
            )?.range;
        }
