...
let DiffEditorViewZones = class DiffEditorViewZones extends Disposable {
    constructor(
        ...
        ) : undefined)).map((m, reader) => m?.read(reader)));
        ...
        this._register(autorun(reader => {
            const newScrollTopModified = this._originalScrollTop.read(reader) - (this._originalScrollOffsetAnimated.read(undefined) - this._modifiedScrollOffsetAnimated.read(reader)) - (this._originalTopPadding.read(undefined) - this._modifiedTopPadding.read(reader));
        ...
        this._register(autorun(reader => {
            const newScrollTopOriginal = this._modifiedScrollTop.read(reader) - (this._modifiedScrollOffsetAnimated.read(undefined) - this._originalScrollOffsetAnimated.read(reader)) - (this._modifiedTopPadding.read(undefined) - this._originalTopPadding.read(reader));
