...
let UndoRedoService = class UndoRedoService {
    ...
    async _confirmAndExecuteWorkspaceUndo(strResource, element, editStackSnapshot, undoConfirmed) {
        if (element.canSplit() && !this._isPartOfUndoGroup(element)) {
            ...
            const {
                ...
                message: ( localize(2481, "Would you like to undo '{0}' across all files?", element.label)),
                buttons: [{
                    label: ( localize(2482, "&&Undo in {0} Files", editStackSnapshot.editStacks.length)),
                    run: () => UndoChoice.All
                }, {
                    label: ( localize(2483, "Undo this &&File")),
                    run: () => UndoChoice.This
                }],
                cancelButton: {
    ...
    async _confirmAndContinueUndo(strResource, sourceId, element) {
        const result = await this._dialogService.confirm({
            message: ( localize(2485, "Would you like to undo '{0}'?", element.label)),
            primaryButton: ( localize(2486, "&&Yes")),
            cancelButton: ( localize(2487, "No"))
        });
        if (!result.confirmed) {
            return;
        }
