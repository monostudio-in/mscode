...
import { NotSelectableGroupId, ListError } from './list.js';
...
class DOMFocusController {
    constructor(list, view) {
        ...
        const onTab = Event.chain(onKeyDown, $ => $.filter(
    ...
    changeSelection(e) {
        ...
        if (this.isSelectionRangeChangeEvent(e)) {
            ...
            if (selection.length === newSelection.length) {
                const itemsToBeSelected = focusGroupId !== undefined ? this.list.filterIndicesByGroup([...newSelection, focus], focusGroupId) : [...newSelection, focus];
...
class List {
    ...
    get onContextMenu() {
        ...
        const fromKeyDown = Event.chain(this.disposables.add(( new DomEmitter(this.view.domNode, "keydown"))).event, $ => ( ( $.map(e => ( new StandardKeyboardEvent(e)))).filter(
        ...
        const fromKeyUp = Event.chain(this.disposables.add(( new DomEmitter(this.view.domNode, "keyup"))).event, $ => ( ( ( $.forEach(() => didJustPressContextMenuKey = false).map(e => ( new StandardKeyboardEvent(e)))).filter(
            e => e.keyCode === KeyCode.ContextMenu || (e.shiftKey && e.keyCode === KeyCode.F10)
        ).map(e => EventHelper.stop(e, true))).map((
            {
                browserEvent
            }
        ) => {
