...
let ExtHostTesting = class ExtHostTesting extends Disposable {
    ...
    async runControllerTestRequest(req, isContinuous, token) {
        ...
        const excludeTests = ( req.excludeExtIds.map(id => lookup.collection.tree.get(id))).filter(isDefined).filter(exclude => ( includeTests.some(include => include.fullId.compare(exclude.fullId) === TestPosition.IsChild)));
...
ExtHostTesting = ( __decorate([( __param(0, IExtHostRpcService)), ( __param(1, ILogService)), ( __param(2, IExtHostCommands)), ( __param(3, IExtHostDocumentsAndEditors))], ExtHostTesting));
        ...
        const run = {
            ...
            appendOutput: (output, location$1, test) => {
                ...
                this.proxy.$appendOutputToRun(
                    ...
                    location$1 && location.from(location$1),
