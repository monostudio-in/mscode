...
let BaseWindow = class BaseWindow extends Disposable {
    ...
    static async confirmOnShutdown(accessor, reason) {
        ...
        const message = reason === ShutdownReason.QUIT ? (isMacintosh ? ( localize(3794, "Are you sure you want to quit?")) : ( localize(3795, "Are you sure you want to exit?"))) : ( localize(3796, "Are you sure you want to close the window?"));
        const primaryButton = reason === ShutdownReason.QUIT ? (isMacintosh ? ( localize(3797, "&&Quit")) : ( localize(3798, "&&Exit"))) : ( localize(3799, "&&Close Window"));
        const res = await dialogService.confirm({
            message,
            primaryButton,
            checkbox: {
                label: ( localize(3800, "Do not ask me again"))
            }
        });
    ...
    onWillShutdown() {
        Event.toPromise(Event.any(Event.once(( new DomEmitter(mainWindow.document.body, EventType.KEY_DOWN, true)).event), Event.once(( new DomEmitter(mainWindow.document.body, EventType.MOUSE_DOWN, true)).event))).then(async () => {
            ...
            await this.dialogService.prompt({
                ...
                message: ( localize(3801, "An unexpected error occurred that requires a reload of this page.")),
                detail: ( localize(3802, "The workbench was unexpectedly disposed while running.")),
                buttons: [{
                    label: ( localize(3803, "&&Reload")),
                ...
                if (matchesScheme(href, Schemas.http) || matchesScheme(href, Schemas.https)) {
                    ...
                    const showProtocolUrlOpenedDialog = async () => {
                        ...
                        await this.hostService.withExpectedShutdown(() => this.dialogService.prompt({
                            ...
                            message: ( localize(3811, "All done. You can close this tab now.")),
                            detail,
                            buttons,
                            cancelButton: true
                        }));
