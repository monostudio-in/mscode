    ...
    async handleURL(uri, options) {
        ...
        if (!trusted) {
            ...
            const result = await this.dialogService.confirm({
                message: ( localize(16289, "Allow '{0}' extension to open this URI?", extensionDisplayName)),
                checkbox: {
                    label: ( localize(16290, "Do not ask me again for this extension"))
                },
                primaryButton: ( localize(16291, "&&Open")),
                custom: {
                    markdownDetails: [{
    ...
    async handleUnhandledURL(uri, extensionId, options) {
        try {
            await this.commandService.executeCommand("workbench.extensions.installExtension", extensionId, {
                justification: {
                    reason: `${( localize(16292, "This extension wants to open a URI:"))}\n${( uri.toString())}`,
                    action: ( localize(16293, "Open URI"))
                },
                enable: true,
        ...
        if (extension) {
            ...
            const result = await this.dialogService.confirm({
                ...
                primaryButton: ( localize(16295, "&&Reload Window and Open"))
            });
            if (!result.confirmed) {
                return;
            }
...
class ManageAuthorizedExtensionURIsAction extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(16296, "Manage Authorized Extension URIs...")),
            category: ( localize2(16297, "Extensions")),
            menu: {
    ...
    async run(accessor) {
        ...
        if (items.length === 0) {
            await quickInputService.pick([{
                label: ( localize(16298, "There are currently no authorized extension URIs."))
            }]);
            return;
        }
