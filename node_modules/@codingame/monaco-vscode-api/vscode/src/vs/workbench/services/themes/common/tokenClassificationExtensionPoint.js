...
const tokenTypeExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(16802, "Contributes semantic token types.")),
        type: "array",
        items: {
            type: "object",
            properties: {
                id: {
                    type: "string",
                    description: ( localize(16803, "The identifier of the semantic token type")),
                    pattern: typeAndModifierIdPattern,
                    patternErrorMessage: ( localize(16804, "Identifiers should be in the form letterOrDigit[_-letterOrDigit]*"))
                },
                superType: {
                    type: "string",
                    description: ( localize(16805, "The super type of the semantic token type")),
                    pattern: typeAndModifierIdPattern,
                    patternErrorMessage: ( localize(16806, "Super types should be in the form letterOrDigit[_-letterOrDigit]*"))
                },
                description: {
                    type: "string",
                    description: ( localize(16807, "The description of the semantic token type"))
                }
            }
        }
    }
...
const tokenModifierExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(16808, "Contributes semantic token modifiers.")),
        type: "array",
        items: {
            type: "object",
            properties: {
                id: {
                    type: "string",
                    description: ( localize(16809, "The identifier of the semantic token modifier")),
                    pattern: typeAndModifierIdPattern,
                    patternErrorMessage: ( localize(16810, "Identifiers should be in the form letterOrDigit[_-letterOrDigit]*"))
                },
                description: {
                    type: "string",
                    description: ( localize(16811, "The description of the semantic token modifier"))
                }
            }
        }
    }
...
const tokenStyleDefaultsExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(16812, "Contributes semantic token scope maps.")),
        type: "array",
        items: {
            type: "object",
            properties: {
                language: {
                    description: ( localize(16813, "Lists the languge for which the defaults are.")),
                    type: "string"
                },
                scopes: {
    ...
    constructor() {
        ...
        tokenTypeExtPoint.setHandler((extensions, delta) => {
            for (const extension of delta.added) {
                ...
                if (!extensionValue || !Array.isArray(extensionValue)) {
                    collector.error(( localize(16819, "'configuration.semanticTokenType' must be an array")));
                    return;
                }
        ...
        tokenModifierExtPoint.setHandler((extensions, delta) => {
            for (const extension of delta.added) {
                ...
                const collector = extension.collector;
                if (!extensionValue || !Array.isArray(extensionValue)) {
                    collector.error(( localize(16820, "'configuration.semanticTokenModifier' must be an array")));
                    return;
                }
                for (const contribution of extensionValue) {
        ...
        tokenStyleDefaultsExtPoint.setHandler((extensions, delta) => {
            for (const extension of delta.added) {
                ...
                const collector = extension.collector;
                if (!extensionValue || !Array.isArray(extensionValue)) {
                    collector.error(( localize(16821, "'configuration.semanticTokenScopes' must be an array")));
                    return;
                }
                for (const contribution of extensionValue) {
                    if (contribution.language && typeof contribution.language !== "string") {
                        collector.error(( localize(16822, "'configuration.semanticTokenScopes.language' must be a string")));
                        continue;
                    }
