...
let CellOutputElement = class CellOutputElement extends Disposable {
    ...
    _renderMissingRenderer(viewModel, preferredMimeType) {
        if (!viewModel.model.outputs.length) {
            return this._renderMessage(viewModel, ( localize(11565, "Cell has no output")));
        }
    ...
    async _attachToolbar(
        ...
        const pickAction = this.toolbarDisposables.add(( new Action("notebook.output.pickMimetype", ( localize(11567, "Change Presentation")), ThemeIcon.asClassName(mimetypeIcon), undefined, async _context => this._pickActiveMimeTypeRenderer(outputItemDiv, notebookTextModel, kernel, this.output))));
    ...
    async _pickActiveMimeTypeRenderer(outputItemDiv, notebookTextModel, kernel, viewModel) {
        ...
        mimeTypes.forEach((mimeType, index) => {
            if (mimeType.isTrusted) {
                ...
                arr.push({
                    ...
                    description: index === currIndex ? ( localize(11568, "Currently Active")) : undefined
                });
            }
        });
        if (( unsupportedItems.some(m => JUPYTER_RENDERER_MIMETYPES.includes(m.id)))) {
            unsupportedItems.push({
                label: ( localize(11569, "Install additional renderers from the marketplace")),
                id: "installRenderers",
        ...
        picker.placeholder = items.length !== mimeTypes.length ? ( localize(11570, "Select mimetype to render for current output")) : ( localize(11571, "Select mimetype to render for current output"));
    ...
    _generateRendererInfo(renderId) {
        ...
        return localize(11572, "renderer not available");
    }
