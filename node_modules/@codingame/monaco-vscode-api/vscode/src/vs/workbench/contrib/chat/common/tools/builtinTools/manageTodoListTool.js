...
function createManageTodoListToolData() {
    ...
    return {
        ...
        displayName: ( localize(7759, "Manage and track todo items for task planning")),
        userDescription: ( localize(7760, "Manage and track todo items for task planning")),
...
let ManageTodoListTool = class ManageTodoListTool extends Disposable {
    ...
    async prepareToolInvocation(context, _token) {
        ...
        let message;
        if (args.operation === "read") {
            message = ( localize(7761, "Read todo list"));
        ...
        const invocationLabel = message?.replace(/^(Starting|Completed): /i, "") ?? ( localize(7762, "Updating todo list"));
        ...
        return {
            ...
            pastTenseMessage: ( new MarkdownString(message ?? ( localize(7763, "Updated todo list")))),
            toolSpecificData: {
                kind: "todoList",
                todoList: todoList
            }
    ...
    generatePastTenseMessage(currentTodos, newTodos) {
        if (currentTodos.length === 0) {
            return newTodos.length === 1 ? ( localize(7764, "Created 1 todo")) : ( localize(7765, "Created {0} todos", newTodos.length));
        }
        ...
        if (addedTodos.length > 0) {
            return addedTodos.length === 1 ? ( localize(7768, "Added 1 todo")) : ( localize(7769, "Added {0} todos", addedTodos.length));
        }
        return localize(7770, "Updated todo list");
    }
