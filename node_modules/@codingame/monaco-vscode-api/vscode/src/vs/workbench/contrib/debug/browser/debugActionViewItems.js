...
let StartDebugActionViewItem = class StartDebugActionViewItem extends BaseActionViewItem {
    constructor(
        ...
        this.selectBox = ( new SelectBox([], -1, contextViewService, defaultSelectBoxStyles, {
            ariaLabel: ( localize(8406, "Debug Launch Configurations")),
    ...
    render(container) {
        ...
        this.toDispose.push(
            addDisposableListener(selectBoxContainer, EventType.KEY_DOWN, e => {
                const event = ( new StandardKeyboardEvent(e));
    ...
    updateOptions() {
            ...
            if (lastGroup !== presentation?.group) {
                ...
                if (this.debugOptions.length) {
                    this.debugOptions.push({
                        label: SeparatorSelectOption.text,
        ...
        if (this.debugOptions.length === 0) {
            this.debugOptions.push({
                label: ( localize(8407, "No Configurations")),
                handler: async () => false
            });
        }
        this.debugOptions.push({
        ...
        manager.getLaunches().filter(l => !l.hidden).forEach(l => {
            const label = inWorkspace ? ( localize(8408, "Add Config ({0})...", l.name)) : ( localize(8409, "Add Configuration..."));
            this.debugOptions.push({
                label,
                handler: async () => {
    ...
    _setAriaLabel(title) {
        ...
        if (keybinding) {
            ariaLabel = ( localize(8410, "{0}, use ({1}) for accessibility help", ariaLabel, keybinding));
        } else {
            ariaLabel = ( localize(
                8411,
...
StartDebugActionViewItem = ( __decorate([( __param(3, IDebugService)), ( __param(4, IConfigurationService)), ( __param(5, ICommandService)), ( __param(6, IWorkspaceContextService)), ( __param(7, IContextViewService)), ( __param(8, IKeybindingService)), ( __param(9, IHoverService)), ( __param(10, IContextKeyService))], StartDebugActionViewItem));
let FocusSessionActionViewItem = class FocusSessionActionViewItem extends SelectActionViewItem {
    constructor(action, session, debugService, contextViewService, configurationService) {
        super(null, action, [], -1, contextViewService, defaultSelectBoxStyles, {
            ariaLabel: ( localize(8412, "Debug Session")),
