...
class SearchEditorModelFactory {
                ...
                if (!ongoingResolve) {
                    ongoingResolve = (async () => {
                        ...
                        const {
                            text,
                            config
                        } = await instantiationService.invokeFunction(parseSavedSearchEditor, existingFile);
                        return ({
                            resultsModel: modelService.createModel(
                                text ?? "",
                                languageService.createById(SEARCH_RESULT_LANGUAGE_ID),
    ...
    async tryFetchModelFromBackupService(
        resource,
        languageService,
        modelService,
        workingCopyBackupService,
        ...
        if (model) {
            ...
            const {
                text,
                config
            } = parseSerializedSearchEditor(existingFile);
