...
let MarkupCellViewModel = class MarkupCellViewModel extends BaseCellViewModel {
    ...
    layoutChange(state) {
        ...
        if (!this.isInputCollapsed) {
            totalHeight = state.totalHeight === undefined ? (this._layoutInfo.layoutState === CellLayoutState.Uninitialized ? 100 : this._layoutInfo.totalHeight) : state.totalHeight;
        ...
        if (this.getEditState() === CellEditState.Editing) {
            commentOffset =
            notebookLayoutConfiguration.editorToolbarHeight + notebookLayoutConfiguration.cellTopMargin + this._chatHeight + this._editorHeight + this._statusBarHeight;
        } else {
            commentOffset = this._previewHeight;
        }
        this._layoutInfo = {
            fontInfo: state.font || this._layoutInfo.fontInfo,
            editorWidth: state.outerWidth !== undefined ? this.viewContext.notebookOptions.computeMarkdownCellEditorWidth(state.outerWidth) : this._layoutInfo.editorWidth,
...
MarkupCellViewModel = ( __decorate([( __param(5, IConfigurationService)), ( __param(6, ITextModelService)), ( __param(7, IUndoRedoService)), ( __param(8, ICodeEditorService)), ( __param(9, IInlineChatSessionService))], MarkupCellViewModel));
