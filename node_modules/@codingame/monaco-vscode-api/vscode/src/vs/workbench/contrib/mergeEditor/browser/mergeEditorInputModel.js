...
let TempFileMergeEditorInputModel = class TempFileMergeEditorInputModel extends EditorModel {
    ...
    async confirmClose(inputModels) {
        ...
        if (someDirty) {
            ...
            const buttons = [{
                label: hasUnhandledConflicts ? ( localize(10975, "&&Save With Conflicts")) : ( localize(10976, "&&Save")),
                run: () => ConfirmResult.SAVE
            }, {
                label: ( localize(10977, "Do&&n't Save")),
            ...
            choice = (await this.dialogService.prompt({
                ...
                )) : isMany ? ( localize(10980, "The merge results will be lost if you don't save them.")) : ( localize(10981, "The merge result will be lost if you don't save it.")),
                buttons,
                cancelButton: {
    ...
    async save(options) {
        ...
        (async () => {
            const {
                ...
                message: ( localize(10982, "Do you want to accept the merge result?")),
                detail: ( localize(
                    10983,
                    "This will write the merge result to the original file and close the merge editor."
                )),
                primaryButton: ( localize(10984, "&&Accept Merge"))
            });
            if (confirmed) {
                await this.accept();
...
let WorkspaceMergeEditorModeFactory = class WorkspaceMergeEditorModeFactory {
    ...
    static {
        this.FILE_SAVED_SOURCE = SaveSourceRegistry.registerSource("merge-editor.source", ( localize(10985, "Before Resolving Conflicts In Merge Editor")));
    }
    async createInputModel(args) {
        const store = ( new DisposableStore());
    ...
    async confirmClose(inputModels) {
        ...
        if (someDirty) {
            ...
            const {
                ...
                )) : isMany ? ( localize(10990, "Your changes will be lost if you don't save them.")) : ( localize(10991, "Your changes will be lost if you don't save them.")),
                buttons: [{
                    label: someUnhandledConflicts ? ( localize(10992, "&&Save with Conflicts")) : ( localize(10993, "&&Save")),
                    run: () => ConfirmResult.SAVE
                }, {
                    label: ( localize(10994, "Do&&n't Save")),
                    run: () => ConfirmResult.DONT_SAVE
                }],
                cancelButton: {
                    run: () => ConfirmResult.CANCEL
            ...
            const {
                ...
                message: isMany ? ( localize(10995, "Do you want to close {0} merge editors?", inputModels.length)) : ( localize(
                    10996,
                    "Do you want to close the merge editor for {0}?",
                    basename(inputModels[0].resultUri)
                )),
                detail: someUnhandledConflicts ? isMany ? ( localize(10997, "The files contain unhandled conflicts.")) : ( localize(10998, "The file contains unhandled conflicts.")) : undefined,
                primaryButton: someUnhandledConflicts ? ( localize(10999, "&&Close with Conflicts")) : ( localize(11000, "&&Close")),
                checkbox: {
                    label: ( localize(11001, "Do not ask me again"))
                }
            });
            if (checkboxChecked) {
