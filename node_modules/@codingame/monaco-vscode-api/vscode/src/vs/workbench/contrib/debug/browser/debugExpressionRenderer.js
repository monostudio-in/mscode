...
let DebugExpressionRenderer = class DebugExpressionRenderer {
    ...
    renderValue(container, expressionOrValue, options = {}) {
        ...
        if (supportsANSI) {
            container.appendChild(handleANSIOutput(
                value,
                linkDetector,
            ...
            container.appendChild(
                linkDetector.linkify(value, hoverBehavior, false, session?.root, true, options.highlights)
            );
        }
        if (options.hover !== false) {
            ...
            store.add(
                this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), container, () => {
                    const container = $("div");
                    ...
                    if (supportsANSI) {
                        hoverContentsPre.appendChild(handleANSIOutput(
                            value,
                            this.linkDetector,
                            session ? session.root : undefined,
                            options.highlights,
...
DebugExpressionRenderer = ( __decorate([( __param(0, ICommandService)), ( __param(1, IConfigurationService)), ( __param(2, IInstantiationService)), ( __param(3, IHoverService))], DebugExpressionRenderer));
