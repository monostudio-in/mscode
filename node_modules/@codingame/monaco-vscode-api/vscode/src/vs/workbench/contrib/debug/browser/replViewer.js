...
class ReplAccessibilityProvider {
    getWidgetAriaLabel() {
        return localize(8730, "Debug Console");
    }
    getAriaLabel(element) {
        if (element instanceof Variable) {
            return localize(8731, "Variable {0}, value {1}", element.name, element.value);
        }
        if (element instanceof ReplOutputElement || element instanceof ReplEvaluationInput || element instanceof ReplEvaluationResult) {
            return element.value + (element instanceof ReplOutputElement && element.count > 1 ? ( localize(8732, ", occurred {0} times", element.count)) : "");
        }
        if (element instanceof RawObjectReplElement) {
            return localize(8733, "Debug console variable {0}, value {1}", element.name, element.value);
        }
        if (element instanceof ReplGroup) {
            return localize(8734, "Debug console group {0}", element.name);
        }
        return "";
    }
}
