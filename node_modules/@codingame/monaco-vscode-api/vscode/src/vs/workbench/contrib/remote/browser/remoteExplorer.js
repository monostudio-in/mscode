...
let ForwardedPortsView = class ForwardedPortsView extends Disposable {
        ...
        if (this.remoteExplorerService.tunnelModel.forwarded.size > 0) {
            this.activityBadge.value = this.activityService.showViewActivity(TUNNEL_VIEW_ID, {
                badge: ( new NumberBadge(
                    ...
                    n => n === 1 ? ( localize(12162, "1 forwarded port")) : ( localize(12163, "{0} forwarded ports", n))
                ))
            });
        } else {
    ...
    get entry() {
        ...
        const text = `${count}`;
        if (count === 0) {
            tooltip = ( localize(12164, "No Ports Forwarded"));
        } else {
        ...
        return {
            name: ( localize(12166, "Forwarded Ports")),
                    ...
                    this.notificationService.notify({
                        ...
                        actions: {
                            primary: [toAction({
                                ...
                                label: ( localize(12169, "Show Setting")),
                                run: async () => {
...
class OnAutoForwardedAction extends Disposable {
    ...
    linkMessage() {
        return localize(12171, "[See all forwarded ports]({0})", `command:${TunnelPanel.ID}.focus`);
    }
    ...
    makePublicChoice(tunnel) {
        return {
            label: ( localize(12173, "Make Public")),
            run: async () => {
    ...
    elevateChoice(tunnel) {
        return {
            label: ( localize(12174, "Use Port {0} as Sudo...", tunnel.tunnelRemotePort)),
            run: async () => {
                await this.remoteExplorerService.close({
                    host: tunnel.tunnelRemoteHost,
                    port: tunnel.tunnelRemotePort
