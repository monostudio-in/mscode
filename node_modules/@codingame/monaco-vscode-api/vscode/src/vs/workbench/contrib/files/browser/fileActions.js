...
const NEW_FILE_LABEL = ( localize2(9727, "New File..."));
const NEW_FOLDER_COMMAND_ID = "explorer.newFolder";
const NEW_FOLDER_LABEL = ( localize2(9728, "New Folder..."));
const TRIGGER_RENAME_LABEL = ( localize(9729, "Rename..."));
const MOVE_FILE_TO_TRASH_LABEL = ( localize(9730, "Delete"));
const COPY_FILE_LABEL = ( localize(9731, "Copy"));
const PASTE_FILE_LABEL = ( localize(9732, "Paste"));
const FileCopiedContext = ( new RawContextKey("fileCopied", false));
const DOWNLOAD_COMMAND_ID = "explorer.download";
const DOWNLOAD_LABEL = ( localize(9733, "Download..."));
const UPLOAD_COMMAND_ID = "explorer.upload";
const UPLOAD_LABEL = ( localize(9734, "Upload..."));
...
async function deleteFiles(
    ...
    let primaryButton;
    if (useTrash) {
        primaryButton = isWindows ? ( localize(9735, "&&Move to Recycle Bin")) : ( localize(9736, "&&Move to Trash"));
    } else {
        primaryButton = ( localize(9737, "&&Delete"));
    }
    ...
    if (dirtyWorkingCopies.size) {
        ...
        const response = await dialogService.confirm({
            type: "warning",
            message,
            detail: ( localize(9742, "Your changes will be lost if you don't save them.")),
            primaryButton
        });
    ...
    if (!skipConfirm) {
        ...
        if (readonlyResources.length) {
            ...
            const response = await dialogService.confirm({
                type: "warning",
                message,
                detail: ( localize(9746, "The read-only protection will be overridden if you continue.")),
                primaryButton: ( localize(9747, "Continue"))
            });
            if (!response.confirmed) {
                return;
            }
    ...
    let confirmation;
    const deleteDetail = ( distinctElements.some(e => e.isDirectory)) ? ( localize(9748, "This action is irreversible!")) : distinctElements.length > 1 ? ( localize(9749, "You can restore these files using the Undo command.")) : ( localize(9750, "You can restore this file using the Undo command."));
    if (skipConfirm || configurationService.getValue(CONFIRM_DELETE_SETTING_KEY) === false) {
        ...
        detail += detail ? "\n" : "";
        if (isWindows) {
            detail += distinctElements.length > 1 ? ( localize(9751, "You can restore these files from the Recycle Bin.")) : ( localize(9752, "You can restore this file from the Recycle Bin."));
        } else {
            detail += distinctElements.length > 1 ? ( localize(9753, "You can restore these files from the Trash.")) : ( localize(9754, "You can restore this file from the Trash."));
        }
        confirmation = await dialogService.confirm({
            ...
            primaryButton,
            checkbox: {
                label: ( localize(9755, "Do not ask me again"))
            }
        });
    }
    else {
    ...
    try {
        ...
        const options = {
            undoLabel: distinctElements.length > 1 ? ( localize(9756, "Delete {0} files", distinctElements.length)) : ( localize(9757, "Delete {0}", distinctElements[0].name)),
            progressLabel: distinctElements.length > 1 ? ( localize(9758, "Deleting {0} files", distinctElements.length)) : ( localize(9759, "Deleting {0}", distinctElements[0].name))
        };
        ...
        if (useTrash) {
            ...
            primaryButton = ( localize(9762, "&&Delete Permanently"));
        } else {
            errorMessage = toErrorMessage(error, false);
            primaryButton = ( localize(9763, "&&Retry"));
        }
...
function getMoveToTrashMessage(distinctElements) {
    ...
    return {
        message: ( localize(9768, "Are you sure you want to delete '{0}'?", distinctElements[0].name)),
        detail: ""
    };
}
...
async function askForOverwrite(fileService, dialogService, targetResource) {
    ...
    const {
        ...
        primaryButton: ( localize(9775, "&&Replace"))
    });
    return confirmed;
}
class GlobalCompareResourcesAction extends Action2 {
    ...
    static {
        this.LABEL = ( localize2(9776, "Compare Active File With..."));
    }
    constructor() {
        super({
            ...
            metadata: {
                description: ( localize2(9777, "Opens a picker to select a file to diff with the active editor."))
            }
        });
    }
    async run(accessor) {
...
class ToggleAutoSaveAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(9778, "Toggle Auto Save")),
            f1: true,
            category: Categories.File,
            metadata: {
                description: ( localize2(9779, "Toggle the ability to save files automatically after typing"))
            }
        });
    }
    run(accessor) {
...
class SaveAllInGroupAction extends BaseSaveAllAction {
    ...
    static {
        this.LABEL = ( localize(9780, "Save All in Group"));
    }
    get class() {
...
let CloseGroupAction = class CloseGroupAction extends Action {
    ...
    static {
        this.LABEL = ( localize(9781, "Close Group"));
    }
...
class FocusFilesExplorer extends Action2 {
    ...
    static {
        this.LABEL = ( localize2(9782, "Focus on Files Explorer"));
    }
    constructor() {
        super({
            ...
            category: Categories.File,
            metadata: {
                description: ( localize2(9783, "Moves focus to the file explorer view container."))
            }
        });
    }
    async run(accessor) {
...
class ShowActiveFileInExplorer extends Action2 {
    ...
    static {
        this.LABEL = ( localize2(9784, "Reveal Active File in Explorer View"));
    }
    constructor() {
        super({
            ...
            category: Categories.File,
            metadata: {
                description: ( localize2(9785, "Reveals and selects the active file within the explorer view."))
            }
        });
    }
    async run(accessor) {
...
class OpenActiveFileInEmptyWorkspace extends Action2 {
    ...
    static {
        this.LABEL = ( localize2(9786, "Open Active Editor in New Empty Workspace"));
    }
    constructor() {
        super({
            ...
            metadata: {
                description: ( localize2(9787, "Opens the active editor in a new window with no folders open."))
            }
        });
    }
    async run(accessor) {
        ...
        if (fileResource && fileService.hasProvider(fileResource)) {
            ...
            dialogService.error(( localize(9788, "The active editor must contain an openable resource.")));
        }
    }
}
function validateFileName(pathService, item, name, os) {
    ...
    if (!name || name.length === 0 || /^\s+$/.test(name)) {
        return {
            content: ( localize(9789, "A file or folder name must be provided.")),
            severity: Severity$1.Error
        };
    }
    if (name[0] === "/" || name[0] === "\\") {
        return {
            content: ( localize(9790, "A file or folder name cannot start with a slash.")),
            severity: Severity$1.Error
        };
    }
    ...
    if (( names.some(name => /^\s|\s$/.test(name)))) {
        return {
            content: ( localize(9793, "Leading or trailing whitespace detected in file or folder name.")),
...
class CompareNewUntitledTextFilesAction extends Action2 {
    ...
    static {
        this.LABEL = ( localize2(9794, "Compare New Untitled Text Files"));
    }
    constructor() {
        super({
            ...
            category: Categories.File,
            metadata: {
                description: ( localize2(9795, "Opens a new diff editor with two untitled files."))
            }
        });
    }
    async run(accessor) {
...
class CompareWithClipboardAction extends Action2 {
    ...
    static {
        this.LABEL = ( localize2(9796, "Compare Active File with Clipboard"));
    }
    static {
    ...
    async run(accessor) {
        ...
        if (resource && (fileService.hasProvider(resource) || resource.scheme === Schemas.untitled)) {
            ...
            const editorLabel = ( localize(9798, "Clipboard ↔ {0}", name));
            await editorService.openEditor({
                original: {
    ...
    const onSuccess = async value => {
        try {
            ...
            await explorerService.applyBulkEdit([( new ResourceFileEdit(undefined, resourceToCreate, {
                ...
                undoLabel: ( localize(9800, "Create {0}", value)),
                progressLabel: ( localize(9801, "Creating {0}", value)),
                ...
                if (( stat.resource.toString()) !== ( targetResource.toString())) {
                    try {
                        await explorerService.applyBulkEdit([( new ResourceFileEdit(stat.resource, targetResource))], {
                            ...
                            undoLabel: ( localize(9802, "Rename {0} to {1}", stat.name, value)),
                            progressLabel: ( localize(9803, "Renaming {0} to {1}", stat.name, value))
                        });
                        await refreshIfSeparator(value, explorerService);
                    } catch (e) {
...
const pasteFileHandler = async (accessor, fileList) => {
    ...
    if (confirmPasteNative && toPaste.files.length >= 1) {
        ...
        const confirmation = await dialogService.confirm({
            ...
            primaryButton: ( localize(9806, "&&Paste"))
        });
        if (!confirmation.confirmed) {
            return;
        }
        ...
        if (toPaste.type === "paths") {
            const sourceTargetPairs = coalesce(await Promise.all(( toPaste.files.map(async fileToPaste => {
                if (( element.resource.toString()) !== ( fileToPaste.toString()) && isEqualOrParent(element.resource, fileToPaste)) {
                    throw ( new Error(( localize(9807, "File to paste is an ancestor of the destination folder"))));
                }
            ...
            if (sourceTargetPairs.length >= 1) {
                if (pasteShouldMove) {
                    ...
                    const options = {
                        confirmBeforeUndo: configurationService.getValue().explorer.confirmUndo === UndoConfirmLevel.Verbose,
                        progressLabel: sourceTargetPairs.length > 1 ? ( localize(9808, "Moving {0} files", sourceTargetPairs.length)) : ( localize(9809, "Moving {0}", basenameOrAuthority(sourceTargetPairs[0].target))),
                        undoLabel: sourceTargetPairs.length > 1 ? ( localize(9810, "Move {0} files", sourceTargetPairs.length)) : ( localize(9811, "Move {0}", basenameOrAuthority(sourceTargetPairs[0].target)))
                    };
                    await explorerService.applyBulkEdit(resourceFileEdits, options);
                } else {
                    const resourceFileEdits = ( sourceTargetPairs.map(pair => ( new ResourceFileEdit(pair.source, pair.target, {
    ...
    async function applyCopyResourceEdit(targets, resourceFileEdits) {
        ...
        const options = {
            ...
            progressLabel: targets.length > 1 ? ( localize(9813, "Copying {0} files", targets.length)) : ( localize(9814, "Copying {0}", basenameOrAuthority(targets[0]))),
            undoLabel: targets.length > 1 ? ( localize(9815, "Paste {0} files", targets.length)) : ( localize(9816, "Paste {0}", basenameOrAuthority(targets[0])))
        };
        await explorerService.applyBulkEdit(resourceFileEdits, options);
    }
};
...
class SetActiveEditorReadonlyInSession extends BaseSetActiveEditorReadonlyInSession {
    ...
    static {
        this.LABEL = ( localize2(9817, "Set Active Editor Read-only in Session"));
    }
    constructor() {
        super(
...
class SetActiveEditorWriteableInSession extends BaseSetActiveEditorReadonlyInSession {
    ...
    static {
        this.LABEL = ( localize2(9818, "Set Active Editor Writeable in Session"));
    }
    constructor() {
        super(
...
class ToggleActiveEditorReadonlyInSession extends BaseSetActiveEditorReadonlyInSession {
    ...
    static {
        this.LABEL = ( localize2(9819, "Toggle Active Editor Read-only in Session"));
    }
    constructor() {
        super(
...
class ResetActiveEditorReadonlyInSession extends BaseSetActiveEditorReadonlyInSession {
    ...
    static {
        this.LABEL = ( localize2(9820, "Reset Active Editor Read-only in Session"));
    }
    constructor() {
        super(
