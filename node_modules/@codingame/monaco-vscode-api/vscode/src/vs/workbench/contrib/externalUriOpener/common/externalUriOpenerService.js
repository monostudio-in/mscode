...
let ExternalUriOpenerService = class ExternalUriOpenerService extends Disposable {
    ...
    async showOpenerPrompt(openers, targetUri, ctx, token) {
        ...
        items.push({
            label: isWeb ? ( localize(9644, "Open in new browser window")) : ( localize(9645, "Open in default browser")),
            opener: undefined
        }, {
            type: "separator"
        }, {
            label: ( localize(9646, "Configure default opener...")),
            opener: "configureDefault"
        });
        const picked = await this.quickInputService.pick(items, {
            placeHolder: ( localize(9647, "How would you like to open: {0}", (targetUri.toString())))
        });
        if (!picked) {
            return true;
        }
