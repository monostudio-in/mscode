...
class PortColumn {
    constructor() {
        this.label = ( localize(12223, "Port"));
        this.tooltip = ( localize(12224, "The label and remote port number of the forwarded port."));
        this.weight = 1;
        this.templateId = "actionbar";
    }
    project(row) {
...
class LocalAddressColumn {
    constructor() {
        this.label = ( localize(12225, "Forwarded Address"));
        this.tooltip = ( localize(12226, "The address that the forwarded port is available at."));
        this.weight = 1;
        this.templateId = "actionbar";
    }
    project(row) {
    ...
    static getHoverText(localAddress) {
        return function(configurationService) {
            ...
            if (editorConf.multiCursorModifier === "ctrlCmd") {
                if (isMacintosh) {
                    clickLabel = ( localize(12227, "option + click"));
                } else {
                    clickLabel = ( localize(12228, "alt + click"));
                }
            } else {
                if (isMacintosh) {
                    clickLabel = ( localize(12229, "cmd + click"));
                } else {
                    clickLabel = ( localize(12230, "ctrl + click"));
                }
            }
...
class RunningProcessColumn {
    constructor() {
        this.label = ( localize(12231, "Running Process"));
        this.tooltip = ( localize(12232, "The command line of the process that is using the port."));
        this.weight = 2;
        this.templateId = "actionbar";
    }
    project(row) {
...
class OriginColumn {
    constructor() {
        this.label = ( localize(12233, "Origin"));
...
class PrivacyColumn {
    constructor() {
        this.label = ( localize(12235, "Visibility"));
        this.tooltip = ( localize(12236, "The availability of the forwarded port."));
        this.weight = 1;
        this.templateId = "actionbar";
    }
    project(row) {
...
let ActionBarRenderer = class ActionBarRenderer {
    ...
    renderInputBox(templateData, editableData) {
        ...
        const inputBox = ( new InputBox(container, this.contextViewService, {
            ariaLabel: ( localize(12237, "Press Enter to confirm or Escape to cancel.")),
            validationOptions: {
                validation: value => {
...
class TunnelItem {
    ...
    get processDescription() {
        ...
        if (this.runningProcess) {
            ...
            description = ( localize(12238, "Process information unavailable"));
        }
        return description;
    }
    get tooltipPostfix() {
    ...
    get iconTooltip() {
        ...
        if (!isAdd) {
            return `${this.processDescription ? ( localize(12241, "Port has running process.")) : ( localize(12242, "No running process."))}`;
        } else {
            return this.label;
        }
    }
    get portTooltip() {
        const isAdd = this.tunnelType === TunnelType.Add;
        if (!isAdd) {
            return `${this.name ? ( localize(12243, "Port labeled {0}. ", this.name)) : ""}`;
        } else {
            return "";
        }
    }
...
const TunnelViewFocusContextKey = ( new RawContextKey("tunnelViewFocus", false, ( localize(12245, "Whether the Ports view has focus."))));
...
let TunnelPanel = class TunnelPanel extends ViewPane {
    ...
    static {
        this.TITLE = ( localize2(12246, "Ports"));
    }
    constructor(
        viewModel,
        options,
    ...
    createTable() {
        ...
        this.table = this.instantiationService.createInstance(WorkbenchTable, "RemoteTunnels", widgetContainer, ( new TunnelTreeVirtualDelegate(this.remoteExplorerService)), columns, [actionBarRenderer], {
            ...
            accessibilityProvider: {
                ...
                getWidgetAriaLabel: () => ( localize(12247, "Tunnel View"))
            },
...
(function(LabelTunnelAction) {
    ...
    LabelTunnelAction.LABEL = ( localize(12248, "Set Port Label"));
            ...
            if (tunnelContext) {
                ...
                return (new Promise(resolve => {
                    ...
                    remoteExplorerService.setEditable(tunnelItem, TunnelEditId.Label, {
                        ...
                        placeholder: ( localize(12249, "Port label")),
                        startingValue
                    });
                }));
            }
...
const invalidPortString = ( localize(12250, "Forwarded port should be a number or a host:port."));
const maxPortNumber = 65536;
const invalidPortNumberString = ( localize(12251, "Port number must be ≥ 0 and < {0}.", maxPortNumber));
const requiresSudoString = ( localize(12252, "May Require Sudo"));
const alreadyForwarded = ( localize(12253, "Port is already forwarded"));
...
(function(ForwardPortAction) {
    ...
    ForwardPortAction.LABEL = ( localize2(12254, "Forward a Port"));
    ForwardPortAction.TREEITEM_LABEL = ( localize(12255, "Forward Port"));
    const forwardPrompt = ( localize(12256, "Port number or address (eg. 3000 or 10.10.10.10:2000)."));
    ...
    function error(notificationService, tunnelOrError, host, port) {
        if (!tunnelOrError) {
            ...
            notificationService.warn(( localize(12258, "Unable to forward {0}:{1}. {2}", host, port, tunnelOrError)));
        }
    }
...
(function(ClosePortAction) {
    ...
    ClosePortAction.LABEL = ( localize2(12260, "Stop Forwarding Port"));
    function inlineHandler() {
        return async (accessor, arg) => {
    ...
    function commandPaletteHandler() {
        return async accessor => {
            ...
            const result = await quickInputService.pick(picks, {
                placeHolder: ( localize(12261, "Choose a port to stop forwarding"))
            });
...
(function(OpenPortInBrowserAction) {
    ...
    OpenPortInBrowserAction.LABEL = ( localize(12262, "Open in Browser"));
    function handler() {
        return async (accessor, arg) => {
            let key;
            if (isITunnelItem(arg)) {
...
(function(OpenPortInPreviewAction) {
    ...
    OpenPortInPreviewAction.LABEL = ( localize(12263, "Preview in Editor"));
    function handler() {
        return async (accessor, arg) => {
            let key;
            if (isITunnelItem(arg)) {
...
(function(OpenPortInBrowserCommandPaletteAction) {
    ...
    OpenPortInBrowserCommandPaletteAction.LABEL = ( localize(12264, "Open Port in Browser"));
    function handler() {
        return async (accessor, arg) => {
            const remoteExplorerService = accessor.get(IRemoteExplorerService);
            const tunnelService = accessor.get(ITunnelService);
            ...
            if (options.length === 0) {
                options.push({
                    label: ( localize(12265, "No ports currently forwarded. Open the Ports view to get started."))
                });
            } else {
                options.push({
                    label: ( localize(12266, "Open the Ports view..."))
                });
            }
            const picked = await quickPickService.pick(options, {
                placeHolder: ( localize(12267, "Choose the port to open"))
            });
...
(function(CopyAddressAction) {
    ...
    CopyAddressAction.INLINE_LABEL = ( localize(12268, "Copy Local Address"));
    CopyAddressAction.COMMANDPALETTE_LABEL = ( localize(12269, "Copy Forwarded Port Address"));
    ...
    function commandPaletteHandler() {
        return async (accessor, arg) => {
            ...
            const result = await quickInputService.pick(makeTunnelPicks(tunnels, remoteExplorerService, tunnelService), {
                placeHolder: ( localize(12270, "Choose a forwarded port"))
            });
            if (result && result.tunnel) {
...
(function(ChangeLocalPortAction) {
    ...
    ChangeLocalPortAction.LABEL = ( localize(12271, "Change Local Address Port"));
    function validateInput(tunnelService, value, canElevate) {
        if (!value.match(/^[0-9]+$/)) {
            return {
                content: ( localize(12272, "Local port should be a number.")),
                severity: Severity$1.Error
            };
        ...
        return async (accessor, arg) => {
            ...
            if (tunnelContext) {
                ...
                remoteExplorerService.setEditable(tunnelItem, TunnelEditId.LocalPort, {
                    ...
                    placeholder: ( localize(12274, "New local port"))
                });
            }
        };
    }
...
(function(SetTunnelProtocolAction) {
    ...
    SetTunnelProtocolAction.LABEL_HTTP = ( localize(12275, "HTTP"));
    SetTunnelProtocolAction.LABEL_HTTPS = ( localize(12276, "HTTPS"));
...
MenuRegistry.appendMenuItem(MenuId.TunnelContext, ({
    ...
    title: ( localize(12277, "Port Visibility")),
...
MenuRegistry.appendMenuItem(MenuId.TunnelContext, ({
    ...
    title: ( localize(12278, "Change Port Protocol")),
