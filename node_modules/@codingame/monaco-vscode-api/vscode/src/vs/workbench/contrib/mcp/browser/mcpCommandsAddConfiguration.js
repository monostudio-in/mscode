...
const AssistedTypes = {
    [AddConfigurationType.NpmPackage]: {
        title: ( localize(10587, "Enter NPM Package Name")),
        placeholder: ( localize(10588, "Package name (e.g., @org/package)")),
        pickLabel: ( localize(10589, "NPM Package")),
        pickDescription: ( localize(10590, "Install from an NPM package name")),
        enabledConfigKey: null
    },
    [AddConfigurationType.PipPackage]: {
        title: ( localize(10591, "Enter Pip Package Name")),
        placeholder: ( localize(10592, "Package name (e.g., package-name)")),
        pickLabel: ( localize(10593, "Pip Package")),
        pickDescription: ( localize(10594, "Install from a Pip package name")),
        enabledConfigKey: null
    },
    [AddConfigurationType.NuGetPackage]: {
        title: ( localize(10595, "Enter NuGet Package Name")),
        placeholder: ( localize(10596, "Package name (e.g., Package.Name)")),
        pickLabel: ( localize(10597, "NuGet Package")),
        pickDescription: ( localize(10598, "Install from a NuGet package name")),
        enabledConfigKey: "chat.mcp.assisted.nuget.enabled"
    },
    [AddConfigurationType.DockerImage]: {
        title: ( localize(10599, "Enter Docker Image Name")),
        placeholder: ( localize(10600, "Image name (e.g., mcp/imagename)")),
        pickLabel: ( localize(10601, "Docker Image")),
        pickDescription: ( localize(10602, "Install from a Docker image")),
        enabledConfigKey: null
    }
};
...
let McpAddConfigurationCommand = class McpAddConfigurationCommand {
    ...
    async getServerType() {
        const items = [{
            ...
            label: ( localize(10603, "Command (stdio)")),
            description: ( localize(10604, "Run a local command that implements the MCP protocol"))
        }, {
            kind: AddConfigurationType.HTTP,
            label: ( localize(10605, "HTTP (HTTP or Server-Sent Events)")),
            description: ( localize(10606, "Connect to a remote HTTP server that implements the MCP protocol"))
        }];
        let aiSupported;
        try {
        ...
        if (aiSupported) {
            items.unshift({
                type: "separator",
                label: ( localize(10607, "Manual Install"))
            });
            ...
            items.push({
                type: "separator",
                label: ( localize(10608, "Model-Assisted"))
            }, ...elligableTypes);
        }
        items.push({
            type: "separator"
        ...
        if (discovery && typeof discovery === "object" && ( allDiscoverySources.some(d => !discovery[d]))) {
            items.push({
                kind: "discovery",
                label: ( localize(10609, "Add from another application..."))
            });
        }
        items.push({
            kind: "browse",
            label: ( localize(10610, "Browse MCP Servers..."))
        });
        const result = await this._quickInputService.pick(items, {
            placeHolder: ( localize(10611, "Choose the type of MCP server to add"))
        });
    ...
    async getStdioConfig() {
        const command = await this._quickInputService.input({
            title: ( localize(10612, "Enter Command")),
            placeHolder: ( localize(10613, "Command to run (with optional arguments)")),
    ...
    async getSSEConfig() {
        const url = await this._quickInputService.input({
            title: ( localize(10614, "Enter Server URL")),
            placeHolder: ( localize(10615, "URL of the MCP server (e.g., http://localhost:3000)")),
            ignoreFocusLost: true
        });
        if (!url) {
            return undefined;
    ...
    async getServerId(suggestion = `my-mcp-server-${generateUuid().split("-")[0]}`) {
        const id = await this._quickInputService.input({
            title: ( localize(10616, "Enter Server ID")),
            placeHolder: ( localize(10617, "Unique identifier for this server")),
            value: suggestion,
            ignoreFocusLost: true
        });
        return id;
    ...
    async getConfigurationTarget() {
        const options = [{
            ...
            label: ( localize(10618, "Global")),
            description: ( localize(10619, "Available in all workspaces, runs locally"))
        }];
        ...
        if (raLabel) {
            options.push({
                ...
                label: ( localize(10620, "Remote")),
                description: ( localize(10621, "Available on this remote machine, runs on {0}", raLabel))
            });
        }
        ...
        if (workbenchState !== WorkbenchState.EMPTY) {
            ...
            if (this._environmentService.remoteAuthority) {
                options.push({
                    target,
                    label: ( localize(10622, "Workspace")),
                    description: ( localize(10623, "Available in this workspace, runs on {0}", raLabel))
                });
            } else {
                options.push({
                    target,
                    label: ( localize(10622, "Workspace")),
                    description: ( localize(10624, "Available in this workspace, runs locally"))
                });
            }
        }
        if (options.length === 1) {
        ...
        const targetPick = await this._quickInputService.pick(options, {
            title: ( localize(10625, "Add MCP Server")),
            placeHolder: ( localize(10626, "Select the configuration target"))
        });
    ...
    async getAssistedConfig(type) {
        ...
        loadingQuickPick.title = ( localize(10627, "Loading package details..."));
            ...
            if (!result || result.state === "error") {
                ...
                if (result?.helpUri) {
                    items.push({
                        ...
                        label: result.helpUriLabel ?? ( localize(10628, "Open help URL")),
                        helpUri: ( URI.parse(result.helpUri))
                    });
                }
                items.push({
                    id: LoadAction.Retry,
                    label: ( localize(10629, "Try a different package"))
                }, {
                    id: LoadAction.Cancel,
                    label: ( localize(10630, "Cancel"))
                });
    ...
    async pickForUrlHandler(resource, showIsPrimary = false) {
        ...
        const placeHolder = ( localize(10633, "Install MCP server {0}", name));
        const items = [{
            id: "install",
            label: ( localize(10634, "Install Server"))
        }, {
            id: "show",
            label: ( localize(10635, "Show Configuration", name))
        }, {
            id: "rename",
            label: ( localize(10636, "Rename \"{0}\"", name))
        }, {
            id: "cancel",
            label: ( localize(10630, "Cancel"))
        }];
        ...
        case "install":
            ...
            try {
                ...
                this._notificationService.error(( localize(10637, "Error installing MCP server {0}: {1}", name, e.message)));
                await this._editorService.openEditor({
                    resource
                });
            }
        ...
        case "rename":
            {
                const newName = await this._quickInputService.input({
                    placeHolder: ( localize(10638, "Enter new name")),
                    value: name
                });
                if (newName) {
...
let McpInstallFromManifestCommand = class McpInstallFromManifestCommand {
    ...
    async run() {
        const result = await this._fileDialogService.showOpenDialog({
            title: ( localize(10639, "Select MCP Server Manifest")),
            filters: [{
                name: ( localize(10640, "MCP Manifest")),
                extensions: ["json"]
            }],
            canSelectFiles: true,
            canSelectMany: false,
            openLabel: ( localize(10641, "&&Install"))
        });
        if (!result?.[0]) {
            return;
        }
        ...
        try {
            ...
            this._notificationService.error(( localize(10642, "Failed to read manifest file: {0}", e.message)));
            return;
        }
        if (!manifest || typeof manifest !== "object") {
            this._notificationService.error(( localize(10643, "Invalid manifest file: expected a JSON object")));
            return;
        }
        ...
        try {
            ...
            this._notificationService.error(( localize(10645, "Failed to parse manifest: {0}", e.message)));
            return;
        }
        let name = galleryManifest.name;
        if (!name) {
            name = await this._quickInputService.input({
                title: ( localize(10646, "Enter Server ID")),
                placeHolder: ( localize(10647, "Unique identifier for this server")),
        ...
        try {
            ...
            this._notificationService.info(( localize(10648, "MCP server '{0}' installed successfully", name)));
        } catch (e) {
            this._notificationService.error(( localize(10649, "Failed to install MCP server: {0}", e.message)));
        }
    }
};
