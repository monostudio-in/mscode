...
let ResourcesDropHandler = class ResourcesDropHandler {
    ...
    async handleDrop(event, targetWindow, resolveTargetGroup, afterDrop, options) {
        ...
        if (this.options.allowWorkspaceOpen) {
            const localFilesAllowedToOpenAsWorkspace = coalesce(( editors.filter(
        ...
        if (externalLocalFiles.length) {
            this.workspacesService.addRecentlyOpened(( externalLocalFiles.map(resource => ({
                fileUri: resource
            }))));
        }
...
ResourcesDropHandler = ( __decorate([( __param(1, IFileService)), ( __param(2, IWorkspacesService)), ( __param(3, IEditorService)), ( __param(4, IWorkspaceEditingService)), ( __param(5, IHostService)), ( __param(6, IWorkspaceContextService)), ( __param(7, IInstantiationService))], ResourcesDropHandler));
function fillEditorsDragData(accessor, resourcesOrEditors, event, options) {
    ...
    const resources = coalesce(( resourcesOrEditors.map(resourceOrEditor => {
        ...
        if (isEditorIdentifier(resourceOrEditor)) {
            if (URI.isUri(resourceOrEditor.editor.resource)) {
                return {
                    resource: resourceOrEditor.editor.resource
                };
            }
            return undefined;
        }
    ...
    if (!options?.disableStandardTransfer) {
        ...
        event.dataTransfer.setData(DataTransfers.TEXT, ( fileSystemResources.map((
            {
                resource
            }
    ...
    const files = fileSystemResources.filter((
        {
            isDirectory
        }
    ) => !isDirectory);
    if (files.length) {
        event.dataTransfer.setData(DataTransfers.RESOURCES, JSON.stringify(( files.map((
            {
                resource
            }
        ) => ( resource.toString())))));
    ...
    for (const resourceOrEditor of resourcesOrEditors) {
        ...
        if (isEditorIdentifier(resourceOrEditor)) {
            ...
            const {
                selection,
                uri
            } = extractSelection(resourceOrEditor);
            editor = {
                resource: uri,
                options: selection ? {
                    selection
    ...
    const draggedDirectories = ( fileSystemResources.filter((
        {
            isDirectory
        }
    ) => isDirectory).map((
...
ResourceListDnDHandler = ( __decorate([( __param(1, IInstantiationService))], ResourceListDnDHandler));
class GlobalWindowDraggedOverTracker extends Disposable {
    ...
    registerListeners() {
        this._register(Event.runAndSubscribe(onDidRegisterWindow, (
            ...
            disposables.add(
                addDisposableListener(window, EventType.DRAG_OVER, () => this.markDraggedOver(false), true)
            );
            disposables.add(
