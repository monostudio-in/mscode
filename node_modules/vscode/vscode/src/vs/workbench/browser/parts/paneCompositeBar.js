...
let PaneCompositeBar = class PaneCompositeBar extends Disposable {
    ...
    getContextMenuActionsForComposite(compositeId) {
        ...
        actions.push(( new SubmenuAction("moveToMenu", ( localize(3668, "Move To")), moveActions)));
        if (defaultLocation !== currentLocation) {
            actions.push(toAction({
                id: "resetLocationAction",
                label: ( localize(3669, "Reset Location")),
                run: () => {
    ...
    createMoveAction(viewContainer, newLocation, defaultLocation) {
        return toAction({
            ...
            label: newLocation === ViewContainerLocation.Panel ? ( localize(3670, "Panel")) : newLocation === ViewContainerLocation.Sidebar ? ( localize(3671, "Primary Side Bar")) : ( localize(3672, "Secondary Side Bar")),
            run: () => {
                let index;
    ...
    getCompositeActions(compositeId) {
        ...
        if (!compositeActions) {
            ...
            if (viewContainer) {
                const viewContainerModel = this.viewDescriptorService.getViewContainerModel(viewContainer);
                compositeActions = {
                    activityAction: this._register(this.instantiationService.createInstance(
                ...
                compositeActions = {
                    activityAction: this._register(
                        this.instantiationService.createInstance(PlaceHolderViewContainerActivityAction, this.toCompositeBarActionItem(
                            compositeId,
                            cachedComposite?.name ?? compositeId,
                            cachedComposite?.icon,
                            undefined
                        ), this.part, this.paneCompositePart)
                    ),
