...
const nlsSingleSelectionRange = ( localize(3510, "Ln {0}, Col {1} ({2} selected)"));
const nlsSingleSelection = ( localize(3511, "Ln {0}, Col {1}"));
const nlsMultiSelectionRange = ( localize(3512, "{0} selections ({1} characters selected)"));
const nlsMultiSelection = ( localize(3513, "{0} selections"));
const nlsEOLLF = ( localize(3514, "LF"));
const nlsEOLCRLF = ( localize(3515, "CRLF"));
let EditorStatus = class EditorStatus extends Disposable {
    ...
    async showIndentationPicker() {
        ...
        if (!activeTextEditorControl) {
            return this.quickInputService.pick([{
                label: ( localize(3516, "No text editor active at this time"))
            }]);
        }
        if (this.editorService.activeEditor?.isReadonly()) {
            return this.quickInputService.pick([{
                label: ( localize(3517, "The active code editor is read-only."))
            }]);
        }
        const picks = ( [
        ...
        picks.splice(3, 0, {
            type: "separator",
            label: ( localize(3518, "convert file"))
        });
        picks.unshift({
            type: "separator",
            label: ( localize(3519, "change view"))
        });
        const action = await this.quickInputService.pick(picks, {
            placeHolder: ( localize(3520, "Select Action")),
            matchOnDetail: true
        });
        return action?.run();
    }
    updateTabFocusModeElement(visible) {
        if (visible) {
            if (!this.tabFocusModeElement.value) {
                const text = ( localize(3521, "Tab Moves Focus"));
                this.tabFocusModeElement.value = this.statusbarService.addEntry({
                    name: ( localize(3522, "Accessibility Mode")),
                    text,
                    ariaLabel: text,
                    tooltip: ( localize(3523, "Disable Accessibility Mode")),
    ...
    updateInputModeElement(inputMode) {
        if (inputMode === "overtype") {
            if (!this.inputModeElement.value) {
                const text = ( localize(3524, "OVR"));
                const name = ( localize(3525, "Enable Insert Mode"));
    ...
    updateColumnSelectionModeElement(visible) {
        if (visible) {
            if (!this.columnSelectionModeElement.value) {
                const text = ( localize(3526, "Column Selection"));
                this.columnSelectionModeElement.value = this.statusbarService.addEntry({
                    name: ( localize(3527, "Column Selection Mode")),
                    text,
                    ariaLabel: text,
                    tooltip: ( localize(3528, "Disable Column Selection Mode")),
    ...
    updateSelectionElement(text) {
        ...
        const props = {
            name: ( localize(3529, "Editor Selection")),
            text,
            ariaLabel: text,
            tooltip: ( localize(3530, "Go to Line/Column")),
    ...
    updateIndentationElement(text) {
        ...
        const props = {
            name: ( localize(3531, "Editor Indentation")),
            text,
            ariaLabel: text,
            tooltip: ( localize(3532, "Select Indentation")),
    ...
    updateEncodingElement(text) {
        ...
        const props = {
            name: ( localize(3533, "Editor Encoding")),
            text,
            ariaLabel: text,
            tooltip: ( localize(3534, "Select Encoding")),
    ...
    updateEOLElement(text) {
        ...
        const props = {
            name: ( localize(3535, "Editor End of Line")),
            text,
            ariaLabel: text,
            tooltip: ( localize(3536, "Select End of Line Sequence")),
    ...
    updateLanguageIdElement(text) {
        ...
        const props = {
            name: ( localize(3537, "Editor Language")),
            text,
            ariaLabel: text,
            tooltip: ( localize(3538, "Select Language Mode")),
    ...
    updateMetadataElement(text) {
        ...
        const props = {
            name: ( localize(3539, "File Information")),
            text,
            ariaLabel: text,
            tooltip: ( localize(3540, "File Information"))
        };
        this.updateElement(
    ...
    onIndentationChange(editorWidget) {
        ...
        if (editorWidget) {
            ...
            if (model) {
                ...
                update.indentation = (modelOpts.insertSpaces ? modelOpts.tabSize === modelOpts.indentSize ? ( localize(3541, "Spaces: {0}", modelOpts.indentSize)) : ( localize(
                    3542,
                    "Spaces: {0} (Tab Size: {1})",
                    modelOpts.indentSize,
                    modelOpts.tabSize
                )) : ( localize(3543, "Tab Size: {0}", modelOpts.tabSize)));
            }
        }
            ...
            if (this.currentMarker) {
                ...
                if (!this.statusBarEntryAccessor.value) {
                    this.statusBarEntryAccessor.value = this.statusbarService.addEntry({
                        name: ( localize(3544, "Current Problem")),
                        text,
                        ariaLabel: text
...
class ChangeLanguageAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(3545, "Change Language Mode")),
            f1: true,
            keybinding: {
            ...
            metadata: {
                description: ( localize(3546, "Change the language mode of the active text editor.")),
                args: [{
                    name: ( localize(3547, "The name of the language mode to change to.")),
    ...
    async run(accessor, languageMode) {
        ...
        const picks = ( languages.map((
            ...
            if (currentLanguageName === languageName) {
                description = ( localize(3548, "({0}) - Configured Language", languageId));
            } else {
                description = ( localize(3549, "({0})", languageId));
            }
            return {
                id: languageId,
                label: languageName,
        ...
        picks.unshift({
            type: "separator",
            label: ( localize(3550, "languages (identifier)"))
        });
        ...
        if (hasLanguageSupport && resource) {
            ...
            if (galleryService.isEnabled()) {
                galleryAction = toAction({
                    ...
                    label: ( localize(3551, "Search Marketplace Extensions for '{0}'...", ext)),
            ...
            configureLanguageSettings = {
                label: ( localize(3552, "Configure '{0}' language based settings...", currentLanguageName))
            };
            picks.unshift(configureLanguageSettings);
            configureLanguageAssociations = {
                label: ( localize(3553, "Configure File Association for '{0}'...", ext))
            };
            picks.unshift(configureLanguageAssociations);
        }
        const autoDetectLanguage = {
            label: ( localize(3554, "Auto Detect"))
        };
        ...
        const pick = typeof languageMode === "string" ? {
            ...
            placeHolder: ( localize(3555, "Select Language Mode")),
    ...
    configureFileAssociation(resource, languageService, quickInputService, configurationService) {
        ...
        const picks = ( languages.map((
            ...
            return {
                ...
                label: languageName,
                iconClasses: getIconClassesForLanguageId(languageId),
                description: (languageId === currentAssociation) ? ( localize(3556, "Current Association")) : undefined
            };
        }));
        setTimeout(async () => {
            const language = await quickInputService.pick(picks, {
                placeHolder: ( localize(3557, "Select Language Mode to Associate with '{0}'", extension || base))
            });
            if (language) {
...
class ChangeEOLAction extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(3558, "Change End of Line Sequence")),
            f1: true
        });
    }
    async run(accessor) {
        ...
        const eol = await quickInputService.pick(EOLOptions, {
            placeHolder: ( localize(3559, "Select End of Line Sequence")),
...
class ChangeEncodingAction extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(3560, "Change File Encoding")),
            f1: true
        });
    }
    async run(accessor) {
        ...
        if (!encodingSupport) {
            await quickInputService.pick([{
                label: ( localize(3561, "No file active at this time"))
            }]);
            return;
        }
        const saveWithEncodingPick = {
            label: ( localize(3562, "Save with Encoding"))
        };
        const reopenWithEncodingPick = {
            label: ( localize(3563, "Reopen with Encoding"))
        };
        ...
        if (guessedEncoding && configuredEncoding !== guessedEncoding && SUPPORTED_ENCODINGS[guessedEncoding]) {
            ...
            picks.unshift({
                ...
                description: ( localize(3564, "Guessed from content"))
            });
        }
        const encoding = await quickInputService.pick(picks, {
            placeHolder: isReopenWithEncoding ? ( localize(3565, "Select File Encoding to Reopen File")) : ( localize(3566, "Select File Encoding to Save with")),
        ...
        if (typeof encoding.id !== "undefined" && activeEncodingSupport) {
            if (isReopenWithEncoding && editorService.activeEditorPane.input.isDirty()) {
                const {
                    ...
                    detail: ( localize(3568, "This will discard any unsaved changes.")),
                    primaryButton: ( localize(3569, "Discard Changes and Reopen"))
                });
                if (!confirmed) {
                    return;
                }
