...
let GlobalCompositeBar = class GlobalCompositeBar extends Disposable {
    ...
    static {
        this.ACCOUNTS_ICON = registerIcon("accounts-view-bar-icon", Codicon.account, ( localize(3584, "Accounts icon in the view bar.")));
    }
    constructor(
        ...
        this.globalActivityActionBar = this._register(( new ActionBar(this.element, {
                ...
                if (action.id === ACCOUNTS_ACTIVITY_ID) {
                    return this.instantiationService.createInstance(AccountsActivityActionViewItem, this.contextMenuActionsProvider, {
                        ...
                        actions.unshift(...[toAction({
                            id: "hideAccounts",
                            label: ( localize(3585, "Hide Accounts")),
            ...
            ariaLabel: ( localize(3586, "Manage")),
...
let AccountsActivityActionViewItem = class AccountsActivityActionViewItem extends AbstractGlobalActivityActionViewItem {
    ...
    async resolveMainMenuActions(accountsMenu, disposables) {
        ...
        if (!this.initialized) {
            const noAccountsAvailableAction = disposables.add(( new Action("noAccountsAvailable", ( localize(3588, "Loading...")), undefined, false)));
            ...
            for (const providerId of registeredProviders) {
                ...
                if (!accounts) {
                    if (( this.problematicProviders.has(providerId))) {
                        const providerUnavailableAction = disposables.add(( new Action("providerUnavailable", ( localize(3589, "{0} is currently unavailable", provider.label)), undefined, false)));
                ...
                for (const account of accounts) {
                    const manageExtensionsAction = toAction({
                        ...
                        label: ( localize(3590, "Manage Trusted Extensions")),
                        enabled: true,
                    ...
                    if (canUseMcp) {
                        const manageMCPAction = toAction({
                            id: `configureSessions${account.label}`,
                            label: ( localize(3591, "Manage Trusted MCP Servers")),
                            enabled: true,
            ...
            for (const providerId of dynamicProviders) {
                ...
                const manageDynamicAuthProvidersAction = toAction({
                    ...
                    label: ( localize(3593, "Manage Dynamic Authentication Providers...")),
                    enabled: true,
...
function simpleActivityContextMenuActions(storageService, isAccount) {
    ...
    return [...currentElementContextMenuActions, toAction({
