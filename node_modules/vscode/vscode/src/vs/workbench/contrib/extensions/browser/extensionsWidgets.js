    ...
    render() {
        ...
        if (!this.small) {
            this.disposables.add(
                this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), this.container, ( localize(9511, "Install count")))
            );
        }
    }
...
let RatingsWidget = class RatingsWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (this.small) {
            ...
            this.containerHover.update(( localize(9512, "Average rating: {0} out of 5", rating)));
    ...
    render() {
        ...
        if (this.small) {
            ...
            this.containerHover = this.disposables.add(
                this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), this.element, ( localize(9513, "Publisher ({0})", this.extension.publisherDisplayName)))
            );
            append(this.element, publisherDisplayName);
            if (this.extension.publisherDomain?.verified) {
                append(this.element, verifiedPublisher);
...
let SponsorWidget = class SponsorWidget extends ExtensionWidget {
    ...
    render() {
        ...
        const label = $("span", undefined, ( localize(9515, "Sponsor")));
...
let ExtensionKindIndicatorWidget = class ExtensionKindIndicatorWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (this.extension?.private) {
            ...
            if (!this.small) {
                append(this.element, $("span.private-extension-label", undefined, ( localize(9517, "Private Extension"))));
            }
            return;
        }
        if (!this.small) {
        ...
        if (workspaceFolder && this.extension.isWorkspaceScoped) {
            this.element.textContent = ( localize(9518, "Workspace Extension"));
            ...
            this.element.textContent = ( localize(9519, "Local Extension"));
        }
    }
};
...
let SyncIgnoredWidget = class SyncIgnoredWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (this.extension && this.extension.state === ExtensionState.Installed && this.userDataSyncEnablementService.isEnabled() && this.extensionsWorkbenchService.isExtensionIgnoredToSync(this.extension)) {
            ...
            this.disposables.add(
                this.hoverService.setupManagedHover(getDefaultHoverDelegate("mouse"), element, ( localize(9520, "This extension is ignored during sync.")))
            );
...
let ExtensionRestartRequiredWidget = class ExtensionRestartRequiredWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (runtimeState && /restart|reload/i.test(reason)) {
            ...
            append(
                this.container,
                $("span.extension-restart-required-label", undefined, ( localize(9521, "Restart Required")))
            );
            this.disposables.add(
    ...
    render() {
        ...
        if (this.extensionViewState.filters.featureId && this.extension.state === ExtensionState.Installed) {
            ...
            if (feature?.icon && accessData) {
                ...
                featureAccessTimeElement.textContent = ( localize(9522, "{0} reqs", accessData.accessTimes.length));
...
let ExtensionHoverWidget = ExtensionHoverWidget_1 = class ExtensionHoverWidget extends ExtensionWidget {
    ...
    getHoverMarkdown() {
        ...
        if (this.extension.private) {
            markdown.appendMarkdown(`$(${privateExtensionIcon.id}) ${( localize(9517, "Private Extension"))}`);
            addSeparator = true;
        }
        if (this.extension.state === ExtensionState.Installed) {
            ...
            if (this.extension.publisherSponsorLink) {
                ...
                markdown.appendMarkdown(`$(${sponsorIcon.id}) [${( localize(9515, "Sponsor"))}](${this.extension.publisherSponsorLink})`);
                addSeparator = true;
            }
        }
        if (addSeparator) {
        ...
        if (location) {
            if (this.extension.isWorkspaceScoped && this.contextService.isInsideWorkspace(location)) {
                markdown.appendMarkdown(( localize(9518, "Workspace Extension")));
            } else {
                markdown.appendMarkdown(( localize(9519, "Local Extension")));
            }
            markdown.appendText(`\n`);
        }
        ...
        if (this.extension.outdated) {
            markdown.appendMarkdown(( localize(9524, "Latest version:")));
            markdown.appendMarkdown(
        ...
        if (extensionRuntimeStatus || extensionFeaturesAccessData.size || extensionStatus.length || runtimeState || recommendationMessage || preReleaseMessage) {
            ...
            if (extensionRuntimeStatus) {
                if (extensionRuntimeStatus.activationTimes) {
                    ...
                    markdown.appendMarkdown(`${( localize(9525, "Activation time"))}${extensionRuntimeStatus.activationTimes.activationReason.startup ? ` (${( localize(9526, "Startup"))})` : ""}: \`${activationTime}ms\``);
                    markdown.appendText(`\n`);
                }
                if (extensionRuntimeStatus.runtimeErrors.length || extensionRuntimeStatus.messages.length) {
                    ...
                    const errorsLink = extensionRuntimeStatus.runtimeErrors.length ? `[${extensionRuntimeStatus.runtimeErrors.length === 1 ? ( localize(9527, "1 uncaught error")) : ( localize(9528, "{0} uncaught errors", extensionRuntimeStatus.runtimeErrors.length))}](${createCommandUri(
    "extension.open",
    this.extension.identifier.id,
    ExtensionEditorTab.Features
)})` : undefined;
                    const messageLink = extensionRuntimeStatus.messages.length ? `[${extensionRuntimeStatus.messages.length === 1 ? ( localize(9529, "1 message")) : ( localize(9530, "{0} messages", extensionRuntimeStatus.messages.length))}](${createCommandUri(
    "extension.open",
    this.extension.identifier.id,
    ExtensionEditorTab.Features
)})` : undefined;
            ...
            if (extensionFeaturesAccessData.size) {
                ...
                for (const [featureId, accessData] of extensionFeaturesAccessData) {
                    if (accessData?.accessTimes.length) {
                        ...
                        if (feature) {
                            markdown.appendMarkdown(( localize(9531, "{0} usage", feature.label)));
    ...
    static getPreReleaseMessage(extension) {
        ...
        const preReleaseVersionLink = `[${( localize(9533, "Pre-Release version"))}](${createCommandUri(
    "workbench.extensions.action.showPreReleaseVersion",
    extension.identifier.id
)})`;
        return localize(9534, "This extension has a {0} available", preReleaseVersionLink);
    }
};
...
let ExtensionRecommendationWidget = class ExtensionRecommendationWidget extends ExtensionWidget {
    ...
    getRecommendationStatus() {
        ...
        if (extRecommendations[this.extension.identifier.id.toLowerCase()]) {
            ...
            return {
                icon: undefined,
                message: ( localize(9535, "You have chosen not to receive recommendations for this extension."))
            };
        }
        return undefined;
    }
...
}, ( localize(9536, "The icon color for extension ratings.")), false);
const extensionPreReleaseIconColor = registerColor("extensionIcon.preReleaseForeground", {
    dark: "#1d9271",
    light: "#1d9271",
    hcDark: "#1d9271",
    hcLight: textLinkForeground
}, ( localize(9537, "The icon color for pre-release extension.")), false);
registerColor("extensionIcon.sponsorForeground", {
    light: "#B51E78",
    dark: "#D758B3",
    hcDark: null,
    hcLight: "#B51E78"
}, ( localize(9538, "The icon color for extension sponsor.")), false);
const extensionPrivateBadgeBackground = registerColor("extensionIcon.privateForeground", {
    dark: "#ffffff60",
    light: "#00000060",
    hcDark: "#ffffff60",
    hcLight: "#00000060"
}, ( localize(9539, "The icon color for private extensions.")));
