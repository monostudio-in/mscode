        ...
        if (this.gallery) {
            ...
            if (!cache) {
                if (this.gallery.assets.manifest) {
                    ...
                    this.logService.error(( localize(9540, "Manifest is not found")), this.identifier.id);
                }
            }
            return cache;
        }
...
let ExtensionsWorkbenchService = class ExtensionsWorkbenchService extends Disposable {
    ...
    async updateAutoUpdateForAllExtensions(isAutoUpdateEnabled) {
        ...
        const result = await this.dialogService.confirm({
            title: ( localize(9541, "Auto Update Extensions")),
            message: isAutoUpdateEnabled ? ( localize(9542, "Do you want to enable auto update for all extensions?")) : ( localize(9543, "Do you want to disable auto update for all extensions?")),
            detail: ( localize(
                9544,
    ...
    updateExtensionsPinnedState(pinned) {
        return this.progressService.withProgress({
            ...
            title: ( localize(9545, "Updating Extensions Auto Update State"))
    ...
    computeExtensionsNotifications() {
        ...
        if (invalidExtensions.length) {
            if (( invalidExtensions.some(e => e.local && e.local.manifest.engines?.vscode && (!isEngineValid(
                ...
                computedNotificiations.push({
                    message: ( localize(9549, "Invalid extensions detected. Review them.")),
                    severity: Severity$1.Warning,
                    extensions: invalidExtensions,
        ...
        if (!this.configurationService.getValue(AutoRestartConfigurationKey)) {
            ...
            if (restartRequiredExtensions.length) {
                ...
                computedNotificiations.push({
                    message: needsReload ? ( localize(9550, "Extensions require a window reload to take effect.")) : ( localize(9551, "Extensions require a restart to take effect.")),
                    severity: Severity$1.Info,
                    extensions: restartRequiredExtensions,
                    query: "@restartrequired",
                    action: {
                        label: needsReload ? ( localize(9552, "Reload Window")) : ( localize(9553, "Restart Extensions")),
                        run: () => {
                            if (needsReload) {
    ...
    async updateRunningExtensions(
        message = ( localize(9555, "Changing extension enablement")),
        auto = false
    ) {
        const toAdd = [];
        const toRemove = [];
            ...
            if (await this.extensionService.stopExtensionHosts(message, auto)) {
                ...
                if (auto) {
                    this.notificationService.notify({
                        severity: Severity$1.Info,
                        message: ( localize(9556, "Extensions were auto restarted to enable updates.")),
    ...
    getRuntimeState(extension) {
        ...
        const reloadActionLabel = reloadAction === ExtensionRuntimeActionType.ReloadWindow ? ( localize(9557, "reload window")) : ( localize(9558, "restart extensions"));
        if (isUninstalled) {
                ...
                if (isEnabled) {
                    ...
                    if (isSameExtensionRunning) {
                        if (!runningExtension.isUnderDevelopment && (extension.version !== runningExtension.version || extension.local.targetPlatform !== runningExtension.targetPlatform)) {
                            ...
                            return {
                                action: reloadAction,
                                reason: ( localize(9563, "Please {0} to enable the updated extension.", reloadActionLabel))
                            };
                        }
                        if (this.extensionsServers.length > 1) {
                            ...
                            if (extensionInOtherServer) {
                                if (runningExtensionServer === this.extensionManagementServerService.remoteExtensionManagementServer && this.extensionManifestPropertiesService.prefersExecuteOnUI(extension.local.manifest) && extensionInOtherServer.server === this.extensionManagementServerService.localExtensionManagementServer) {
                                    return {
                                        action: reloadAction,
                                        reason: ( localize(9564, "Please {0} to enable this extension locally.", reloadActionLabel))
                                    };
                                }
                        ...
                        if (extension.server === this.extensionManagementServerService.localExtensionManagementServer && runningExtensionServer === this.extensionManagementServerService.remoteExtensionManagementServer) {
                            if (this.extensionManifestPropertiesService.prefersExecuteOnUI(extension.local.manifest)) {
                                return {
                                    action: reloadAction,
                                    reason: ( localize(9566, "Please {0} to enable this extension.", reloadActionLabel))
                                };
                            }
                        }
                    ...
                    if (isSameExtensionRunning) {
                        return {
                            action: reloadAction,
                            reason: ( localize(9567, "Please {0} to disable this extension.", reloadActionLabel))
                        };
                    }
                }
                return undefined;
    ...
    async downloadVSIX(extensionId, versionKind) {
        ...
        if (!galleryExtension) {
            throw ( new Error(( localize(9568, "Extension '{0}' not found.", extensionId))));
        }
        ...
        for (const targetPlatform of version?.targetPlatforms ?? galleryExtension.allTargetPlatforms) {
            if (targetPlatform !== TargetPlatform.UNKNOWN && targetPlatform !== TargetPlatform.UNIVERSAL) {
                options.push({
                    label: targetPlatform === TargetPlatform.UNDEFINED ? ( localize(9569, "All Platforms")) : TargetPlatformToString(targetPlatform),
                    id: targetPlatform
                });
            }
        }
        if (options.length > 1) {
            const message = ( localize(9570, "Please select the platform for which you want to download the VSIX"));
        ...
        const result = await this.fileDialogService.showOpenDialog({
            title: ( localize(9571, "Select folder to download the VSIX")),
            canSelectFiles: false,
            canSelectFolders: true,
            canSelectMany: false,
            openLabel: ( localize(9572, "Download"))
        });
        if (!result?.[0]) {
            return;
        }
        this.progressService.withProgress({
            ...
            try {
                progress.report({
                    message: ( localize(9573, "Downloading VSIX..."))
                });
                ...
                this.notificationService.info(( localize(9574, "Successfully downloaded the VSIX")));
            } catch (error) {
                this.notificationService.error(( localize(9575, "Error while downloading the VSIX: {0}", getErrorMessage(error))));
            }
        });
    }
    async pickVersionToDownload(extensionId) {
        ...
        if (!allVersions.length) {
            await this.dialogService.info(( localize(9576, "This extension has no other versions.")));
            return;
        }
        const picks = ( allVersions.map((v, i) => {
            return {
                id: v.version,
                label: v.version,
                description: `${fromNow(( new Date(Date.parse(v.date))), true)}${v.isPreReleaseVersion ? ` (${( localize(9577, "pre-release"))})` : ""}`,
                ariaLabel: `${v.isPreReleaseVersion ? "Pre-Release version" : "Release version"} ${v.version}`,
                data: v
            };
        }));
        const pick = await this.quickInputService.pick(picks, {
            placeHolder: ( localize(9578, "Select Version to Download")),
            matchOnDetail: true
        });
        return pick?.data;
    }
    ...
    async canInstall(extension) {
        if (!(extension instanceof Extension)) {
            return ( new MarkdownString()).appendText(( localize(9581, "The provided object is not an extension.")));
        }
        if (extension.isMalicious) {
            return ( new MarkdownString()).appendText(( localize(9582, "This extension is reported to be problematic.")));
        }
        if (extension.deprecationInfo?.disallowInstall) {
            return ( new MarkdownString()).appendText(( localize(9583, "This extension is disallowed to be installed.")));
        }
        if (extension.gallery) {
            if (!extension.gallery.isSigned && shouldRequireRepositorySignatureFor(
                ...
                return ( new MarkdownString()).appendText(( localize(9584, "This extension is not signed.")));
            }
    ...
    async install(arg, installOptions = {}, progressLocation) {
        ...
        if (!extension) {
            throw ( new Error(( localize(9586, "Unable to install extension"))));
        }
        if (installOptions.enable) {
            if (extension.enablementState === EnablementState.DisabledWorkspace || extension.enablementState === EnablementState.DisabledGlobally) {
                if (installOptions.justification) {
                    const result = await this.dialogService.confirm({
                        title: ( localize(9587, "Enable Extension")),
                        message: ( localize(9588, "Would you like to enable '{0}' extension?", extension.displayName)),
                        detail: isString(installOptions.justification) ? installOptions.justification : installOptions.justification.reason,
                        primaryButton: isString(installOptions.justification) ? ( localize(9589, "&&Enable Extension")) : ( localize(9590, "&&Enable Extension and {0}", installOptions.justification.action))
                    });
                    if (!result.confirmed) {
    ...
    async _install(arg, installOptions = {}, progressLocation) {
        ...
        if (arg instanceof URI) {
            ...
            if (extension?.isMalicious) {
                throw ( new Error(( localize(9582, "This extension is reported to be problematic."))));
            }
            if (gallery) {
        ...
        if (installable) {
            if (installOptions.justification) {
                ...
                const buttons = [];
                buttons.push({
                    label: isString(installOptions.justification) || !installOptions.justification.action ? ( localize(9594, "&&Install Extension")) : ( localize(9595, "&&Install Extension and {0}", installOptions.justification.action)),
                    run: () => true
                });
                if (!extension) {
                    buttons.push({
                        label: ( localize(9596, "Open Extension")),
                        run: () => {
                            this.open(extension);
                            return false;
                        }
                ...
                const result = await this.dialogService.prompt({
                    title: ( localize(9597, "Install Extension")),
                    message: extension ? ( localize(
                        9598,
                        "Would you like to install '{0}' extension from '{1}'?",
                        extension.displayName,
                        extension.publisherDisplayName
                    )) : ( localize(9599, "Would you like to install the extension?")),
                    detail: isString(installOptions.justification) ? installOptions.justification : installOptions.justification.reason,
                    cancelButton: true,
                    buttons,
                    checkbox: syncCheck ? {
                        label: ( localize(9600, "Sync this extension")),
                        checked: true
                    } : undefined
                });
                if (!result.result) {
    ...
    async uninstall(e) {
        ...
        if (extension.local.isApplicationScoped && this.userDataProfilesService.profiles.length > 1) {
            const {
                ...
                title: ( localize(9602, "Uninstall Extension")),
                ...
                primaryButton: ( localize(9604, "Uninstall (All Profiles)"))
            });
            if (!confirmed) {
                throw ( new CancellationError());
            }
        ...
        return this.withProgress({
            location: ProgressLocation.Extensions,
            title: ( localize(9607, "Uninstalling extension...")),
    ...
    doInstall(extension, installTask, progressLocation) {
        const title = extension ? ( localize(9611, "Installing '{0}' extension...", extension.displayName)) : ( localize(9612, "Installing extension..."));
        return this.withProgress({
