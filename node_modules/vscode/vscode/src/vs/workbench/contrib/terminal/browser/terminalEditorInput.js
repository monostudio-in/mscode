...
let TerminalEditorInput = class TerminalEditorInput extends EditorInput {
    ...
    async confirm(terminals) {
        const {
            ...
            message: ( localize(13538, "Do you want to terminate running processes?")),
            primaryButton: ( localize(13539, "&&Terminate")),
            detail: terminals.length > 1 ? ( terminals.map(terminal => terminal.editor.getName())).join("\n") + "\n\n" + ( localize(13540, "Closing will terminate the running processes in the terminals.")) : ( localize(13541, "Closing will terminate the running processes in this terminal."))
        });
