...
function getTerminalDecorationHoverContent(command, hoverMessage, showCommandActions) {
    let hoverContent = showCommandActions ? `${( localize(13671, "Show Command Actions"))}\n\n---\n\n` : "";
    if (!command) {
        if (hoverMessage) {
        ...
        if (isNumber(command.duration)) {
            ...
            if (command.exitCode) {
                if (command.exitCode === -1) {
                    hoverContent += ( localize(13675, "Command executed {0} and failed", fromNow(command.timestamp, true)));
                } else {
                    hoverContent += ( localize(
                        13676,
                ...
                hoverContent += ( localize(13677, "Command executed {0} now"));
            }
        }
    }
    return hoverContent;
...
const unknownText = ( localize(13678, "Unknown"));
const runningText = ( localize(13679, "Running"));
function getTerminalCommandDecorationTooltip(command, storedState) {
    ...
    if (typeof duration === "number") {
        ...
        if (exitCode) {
            ...
            hoverContent += ( localize(13674, "Command executed {0} and took {1}", fromNowText, durationText));
        }
    } else {
        if (exitCode) {
            if (exitCode === -1) {
                hoverContent += ( localize(13675, "Command executed {0} and failed", fromNowText));
            } else {
                hoverContent += ( localize(
                    13676,
                    "Command executed {0} and failed (Exit Code {1})",
            ...
            hoverContent += ( localize(13680, "Command executed {0} ", fromNowText));
        }
    }
    return hoverContent;
}
