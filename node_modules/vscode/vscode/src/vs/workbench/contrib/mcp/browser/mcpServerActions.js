...
let InstallAction = class InstallAction extends McpServerAction {
    ...
    constructor(open, mcpWorkbenchService, telemetryService, mcpService) {
        super("extensions.install", ( localize(10722, "Install")), InstallAction_1.CLASS, false);
        this.open = open;
...
let InstallInWorkspaceAction = class InstallInWorkspaceAction extends McpServerAction {
    ...
    constructor(
        ...
        super("extensions.installWorkspace", ( localize(10724, "Install in Workspace")), InstallAction.CLASS, false);
        this.open = open;
        this.mcpWorkbenchService = mcpWorkbenchService;
    ...
    async getConfigurationTarget() {
        ...
        for (const folder of this.workspaceService.getWorkspace().folders) {
            options.push({
                target: folder,
                label: folder.name,
                description: ( localize(10725, "Workspace Folder"))
            });
        }
        if (this.workspaceService.getWorkbenchState() === WorkbenchState.WORKSPACE) {
            ...
            options.push({
                ...
                label: ( localize(10726, "Workspace"))
            });
        }
        if (options.length === 1) {
        ...
        const targetPick = await this.quickInputService.pick(options, {
            title: ( localize(10727, "Choose where to install the MCP server"))
        });
...
let InstallInRemoteAction = class InstallInRemoteAction extends McpServerAction {
    ...
    constructor(
        ...
        super("extensions.installRemote", ( localize(10728, "Install (Remote)")), InstallAction.CLASS, false);
        this.open = open;
        this.mcpWorkbenchService = mcpWorkbenchService;
        ...
        this.label = ( localize(10729, "Install in {0}", remoteLabel));
        this.update();
    }
    update() {
        this.enabled = false;
...
class InstallingLabelAction extends McpServerAction {
    static {
        this.LABEL = ( localize(10730, "Installing"));
    }
    static {
...
let UninstallAction = class UninstallAction extends McpServerAction {
    ...
    constructor(mcpWorkbenchService) {
        super("extensions.uninstall", ( localize(10731, "Uninstall")), UninstallAction_1.CLASS, false);
        this.mcpWorkbenchService = mcpWorkbenchService;
        this.update();
    }
    update() {
        ...
        this.enabled = true;
        this.label = ( localize(10731, "Uninstall"));
    }
    async run() {
        if (!this.mcpServer) {
            return;
...
let EnableMcpServerGloballyAction = class EnableMcpServerGloballyAction extends McpServerAction {
    ...
    constructor(mcpService) {
        super(EnableMcpServerGloballyAction_1.ID, ( localize(10732, "Enable")), McpServerAction.LABEL_ACTION_CLASS);
        this.mcpService = mcpService;
        this.tooltip = ( localize(10733, "Enable this MCP server"));
        this.update();
    }
    update() {
        this.enabled = false;
...
let EnableMcpServerForWorkspaceAction = class EnableMcpServerForWorkspaceAction extends McpServerAction {
    ...
    constructor(mcpService, workspaceService) {
        super(EnableMcpServerForWorkspaceAction_1.ID, ( localize(10734, "Enable (Workspace)")), McpServerAction.LABEL_ACTION_CLASS);
        this.mcpService = mcpService;
        this.workspaceService = workspaceService;
        this.tooltip = ( localize(10735, "Enable this MCP server only in this workspace"));
        this.update();
    }
    update() {
        this.enabled = false;
...
let DisableMcpServerGloballyAction = class DisableMcpServerGloballyAction extends McpServerAction {
    ...
    constructor(mcpService) {
        super(DisableMcpServerGloballyAction_1.ID, ( localize(10736, "Disable")), McpServerAction.LABEL_ACTION_CLASS);
        this.mcpService = mcpService;
        this.tooltip = ( localize(10737, "Disable this MCP server"));
        this.update();
    }
    update() {
        this.enabled = false;
...
let DisableMcpServerForWorkspaceAction = class DisableMcpServerForWorkspaceAction extends McpServerAction {
    ...
    constructor(mcpService, workspaceService) {
        super(DisableMcpServerForWorkspaceAction_1.ID, ( localize(10738, "Disable (Workspace)")), McpServerAction.LABEL_ACTION_CLASS);
        this.mcpService = mcpService;
        this.workspaceService = workspaceService;
        this.tooltip = ( localize(10739, "Disable this MCP server only in this workspace"));
        this.update();
    }
    update() {
        this.enabled = false;
...
let ManageMcpServerAction = class ManageMcpServerAction extends DropDownAction {
    ...
    constructor(isEditorAction, instantiationService) {
        ...
        this.tooltip = ( localize(10740, "Manage"));
        this.update();
    }
    async run() {
        return super.run(
...
let StartServerAction = class StartServerAction extends McpServerAction {
    ...
    constructor(mcpService) {
        super("extensions.start", ( localize(10741, "Start Server")), StartServerAction_1.CLASS, false);
        this.mcpService = mcpService;
        this.update();
    }
    update() {
        ...
        this.enabled = true;
        this.label = ( localize(10741, "Start Server"));
    }
    async run() {
        const server = this.getServer();
        if (!server) {
...
let StopServerAction = class StopServerAction extends McpServerAction {
    ...
    constructor(mcpService) {
        super("extensions.stop", ( localize(10742, "Stop Server")), StopServerAction_1.CLASS, false);
        this.mcpService = mcpService;
        this.update();
    }
    update() {
        ...
        this.enabled = true;
        this.label = ( localize(10742, "Stop Server"));
    }
    async run() {
        const server = this.getServer();
        if (!server) {
...
let RestartServerAction = class RestartServerAction extends McpServerAction {
    ...
    constructor(mcpService) {
        super("extensions.restart", ( localize(10743, "Restart Server")), RestartServerAction_1.CLASS, false);
        this.mcpService = mcpService;
        this.update();
    }
    update() {
        ...
        this.enabled = true;
        this.label = ( localize(10743, "Restart Server"));
    }
    async run() {
        const server = this.getServer();
        if (!server) {
...
let AuthServerAction = class AuthServerAction extends McpServerAction {
    ...
    static {
        this.SIGN_OUT = ( localize(10744, "Sign Out"));
    }
    static {
        this.DISCONNECT = ( localize(10745, "Disconnect Account"));
    }
    constructor(mcpService, _authenticationQueryService, _authenticationService) {
        super("extensions.restart", ( localize(10743, "Restart Server")), RestartServerAction.CLASS, false);
        this.mcpService = mcpService;
...
let ShowServerOutputAction = class ShowServerOutputAction extends McpServerAction {
    ...
    constructor(mcpService) {
        super("extensions.output", ( localize(10746, "Show Output")), ShowServerOutputAction_1.CLASS, false);
        this.mcpService = mcpService;
        this.update();
    }
    update() {
        ...
        this.enabled = true;
        this.label = ( localize(10746, "Show Output"));
    }
    async run() {
        const server = this.getServer();
        if (!server) {
...
let ShowServerConfigurationAction = class ShowServerConfigurationAction extends McpServerAction {
    ...
    constructor(mcpWorkbenchService) {
        super("extensions.config", ( localize(10747, "Show Configuration")), ShowServerConfigurationAction_1.CLASS, false);
        this.mcpWorkbenchService = mcpWorkbenchService;
        this.update();
    }
    update() {
...
let ShowServerJsonConfigurationAction = class ShowServerJsonConfigurationAction extends McpServerAction {
    ...
    constructor(mcpService, mcpRegistry, editorService) {
        super("extensions.jsonConfig", ( localize(10748, "Show Configuration (JSON)")), ShowServerJsonConfigurationAction_1.CLASS, false);
        this.mcpService = mcpService;
...
let ConfigureModelAccessAction = class ConfigureModelAccessAction extends McpServerAction {
    ...
    constructor(mcpService, commandService) {
        super("extensions.config", ( localize(10749, "Configure Model Access")), ConfigureModelAccessAction_1.CLASS, false);
        this.mcpService = mcpService;
    ...
    update() {
        ...
        this.enabled = true;
        this.label = ( localize(10749, "Configure Model Access"));
    }
    async run() {
        const server = this.getServer();
        if (!server) {
...
let ShowSamplingRequestsAction = class ShowSamplingRequestsAction extends McpServerAction {
    ...
    constructor(mcpService, samplingService, editorService) {
        super("extensions.config", ( localize(10750, "Show Sampling Requests")), ShowSamplingRequestsAction_1.CLASS, false);
        this.mcpService = mcpService;
    ...
    async run() {
        ...
        this.editorService.openEditor({
            ...
            label: ( localize(10751, "MCP Sampling: {0}", server.definition.label))
        });
    }
    getServer() {
        if (!this.mcpServer) {
...
let BrowseResourcesAction = class BrowseResourcesAction extends McpServerAction {
    ...
    constructor(mcpService, commandService) {
        super("extensions.config", ( localize(10752, "Browse Resources")), BrowseResourcesAction_1.CLASS, false);
        this.mcpService = mcpService;
        this.commandService = commandService;
        this.update();
    }
