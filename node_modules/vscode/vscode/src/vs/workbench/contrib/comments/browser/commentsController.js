    ...
    _findNearestCommentingRange(reverse) {
        ...
        if (this.accessibilityService.isScreenReaderOptimized()) {
            ...
            if (commentRangeStart && commentRangeEnd) {
                ...
                oneLine ? status(( localize(8061, "Line {0}", commentRangeStart))) : status(( localize(8062, "Lines {0} to {1}", commentRangeStart, commentRangeEnd)));
            }
        }
    }
    nextCommentingRange() {
            ...
            if (!this._hasRespondedToEditorChange) {
                if (( this._commentInfos.some(
                    ...
                    if (verbose) {
                        ...
                        status(( localize(8065, "Editor has commenting ranges.")));
                    }
                }
            }
        });
    ...
    addCommentAtLine(range, e) {
        ...
        if (!newCommentInfos.length || !this.editor?.hasModel()) {
            ...
            if (!newCommentInfos.length) {
                if (range) {
                    this.notificationService.error(( localize(8066, "The cursor must be within a commenting range to add a comment.")));
                } else {
                    this.notificationService.error(( localize(8067, "File comments are not allowed on this file.")));
                }
            }
            return Promise.resolve();
        }
        if (newCommentInfos.length > 1) {
            if (e && range) {
                ...
                return this.quickInputService.pick(picks, {
                    placeHolder: ( localize(8068, "Select Comment Provider")),
