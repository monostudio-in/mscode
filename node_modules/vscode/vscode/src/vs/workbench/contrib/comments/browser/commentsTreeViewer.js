...
const COMMENTS_VIEW_TITLE = ( localize2(8082, "Comments"));
...
let CommentNodeRenderer = class CommentNodeRenderer {
    ...
    getCountString(commentCount) {
        if (commentCount > 2) {
            return localize(8083, "{0} replies", commentCount - 1);
        } else if (commentCount === 2) {
            return localize(8084, "1 reply");
        } else {
            return localize(8085, "1 comment");
        }
    }
    getRenderedComment(commentBody) {
        ...
        for (let i = 0; i < images.length; i++) {
            ...
            textDescription.textContent = image.alt ? ( localize(8086, "Image: {0}", image.alt)) : ( localize(8087, "Image"));
    ...
    renderElement(node, index, templateData) {
        ...
        if (node.element.threadRelevance === CommentThreadApplicability.Outdated) {
            ...
            templateData.threadMetadata.relevance.innerText = ( localize(8088, "Outdated"));
        ...
        if (node.element.range) {
            if (node.element.range.startLineNumber === node.element.range.endLineNumber) {
                templateData.threadMetadata.range.textContent = ( localize(8089, "[Ln {0}]", node.element.range.startLineNumber));
            } else {
        ...
        templateData.repliesMetadata.lastReplyDetail.textContent = ( localize(8091, "Last reply from {0}", lastComment.userName));
