...
function toKernelQuickPick(kernel, selected) {
    ...
    if (kernel.id === selected?.id) {
        if (!res.description) {
            res.description = ( localize(11593, "Currently Selected"));
        } else {
            res.description = ( localize(11594, "{0} - Currently Selected", res.description));
        }
    }
    return res;
}
    ...
    async _getKernelRecommendationsQuickPickItems(notebookTextModel, extensionWorkbenchService) {
        ...
        if (suggestedExtension) {
            ...
            quickPickItems.push({
                ...
                label: `$(${Codicon.lightbulb.id}) ` + ( localize(11597, "Install/Enable suggested extensions")),
                extensionIds: suggestedExtension.extensionIds
            });
        }
        quickPickItems.push({
            id: "install",
            label: ( localize(11598, "Browse marketplace for kernel extensions"))
        });
...
let KernelPickerMRUStrategy = class KernelPickerMRUStrategy extends KernelPickerStrategyBase {
    ...
    _getKernelPickerQuickPickItems(
        ...
        quickPickItems.push({
            id: "selectAnother",
            label: ( localize(11599, "Select Another Kernel...")),
    ...
    async displaySelectAnotherQuickPick(editor, kernelListEmpty) {
        ...
        const quickPickItem = await ( new Promise(resolve => {
            quickPick.title = kernelListEmpty ? ( localize(11600, "Select Kernel")) : ( localize(11601, "Select Another Kernel"));
            quickPick.placeholder = ( localize(11602, "Type to choose a kernel source"));
            quickPick.busy = true;
    ...
    async _calculdateKernelSources(editor) {
        ...
        quickPickItems.push(...( validActions.map(action => {
            const buttons = action.documentation ? [{
                ...
                tooltip: ( localize(11603, "Learn More"))
            }] : [];
            return {
    ...
    async _selectOneKernel(notebook, source, kernels) {
        ...
        quickPick.items = quickPickItems;
        quickPick.canSelectMany = false;
        quickPick.title = ( localize(11604, "Select Kernel from {0}", source));
    ...
    static updateKernelStatusAction(notebook, action, notebookKernelService, notebookKernelHistoryService) {
        ...
        if (detectionTasks.length) {
            ...
            if (info.selected) {
                ...
                action.tooltip = kernelInfo ? ( localize(11605, "Selected Kernel: {0} (Kernel Detection Tasks Running)", kernelInfo)) : ( localize(11606, "Detecting Kernels"));
            } else {
                action.label = ( localize(11606, "Detecting Kernels"));
            }
            return;
        }
        ...
        if (selected) {
            ...
            action.label = ( localize(11600, "Select Kernel"));
            action.class = ThemeIcon.asClassName(selectKernelIcon);
            action.tooltip = "";
        }
    }
