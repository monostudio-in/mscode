        ...
        for (let i = 0; i < str.length; i++) {
            if (str[i] === "\\") {
                ...
                if (escaped === "n") {
                    ...
                    throw Error(( localize(12913, "All backslashes in Query string must be escaped (\\\\)")));
                }
            } else {
                out += str[i];
            }
...
const serializeSearchResultForEditor = (
    ...
    const filecount = searchResult.fileCount() > 1 ? ( localize(12914, "{0} files", searchResult.fileCount())) : ( localize(12915, "1 file"));
    const resultcount = searchResult.count() > 1 ? ( localize(12916, "{0} results", searchResult.count())) : ( localize(12917, "1 result"));
    const info = [searchResult.count() ? `${resultcount} - ${filecount}` : ( localize(12918, "No Results"))];
    if (limitHit) {
        info.push(( localize(
            12919,
