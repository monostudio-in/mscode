...
class ExpressionContainer {
        ...
        try {
            ...
            const vars = ( response.body.variables.filter(v => !!v).map(v => {
                ...
                return (new Variable(
                    ...
                    "",
                    undefined,
                    localize(8851, "Invalid variable attributes"),
                    0,
                    0,
                    undefined,
                    {
    ...
    async evaluateExpression(expression, session, stackFrame, context, keepLazyVars = false, location) {
        if (!session || (!stackFrame && context !== "repl")) {
            this.value = context === "repl" ? ( localize(8852, "Please start a debug session to evaluate expressions")) : Expression.DEFAULT_VALUE;
            this.reference = 0;
            return false;
        }
        this.session = session;
...
class Expression extends ExpressionContainer {
    static {
        this.DEFAULT_VALUE = ( localize(8853, "not available"));
    }
...
class Thread {
    ...
    get stateLabel() {
        if (this.stoppedDetails) {
            return this.stoppedDetails.description || (this.stoppedDetails.reason ? ( localize(8854, "Paused on {0}", this.stoppedDetails.reason)) : ( localize(8855, "Paused")));
        }
        return localize(8856, "Running");
    }
