        ...
        if (!isCancellationError(error)) {
            ...
            if (editorsToRevert.length > 0) {
                actions.push(toAction({
                    ...
                    label: editorsToRevert.length > 1 ? ( localize(9822, "Revert All")) : ( localize(9823, "Revert")),
            ...
            notificationService.notify({
                ...
                message: ( localize(9824, "Failed to save '{0}': {1}", ( editors.map((
                    {
                        editor
                    }
...
CommandsRegistry.registerCommand({
    ...
    handler: async accessor => {
        ...
        try {
            ...
            const notificationService = accessor.get(INotificationService);
            notificationService.error(( localize(9825, "Failed to revert '{0}': {1}", ( editors.map((
                {
                    editor
                }
            ) => editor.getName())).join(", "), toErrorMessage(error, false))));
