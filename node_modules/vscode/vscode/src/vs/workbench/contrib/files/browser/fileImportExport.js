            ...
            return uploadLimiter.queue(async () => {
                ...
                if (target && entry.name && target.getChild(entry.name)) {
                    ...
                    await this.explorerService.applyBulkEdit([( new ResourceFileEdit(joinPath(target.resource, entry.name), undefined, {
                        ...
                        undoLabel: ( localize(9835, "Overwrite {0}", entry.name)),
                        progressLabel: ( localize(9836, "Overwriting {0}", entry.name))
                    });
                    if (token.isCancellationRequested) {
                        return;
                    }
...
let ExternalFileImport = class ExternalFileImport {
    ...
    async doImport(target, source, targetWindow, token) {
        ...
        if (folders.length > 0 && target.isRoot) {
            ...
            const buttons = [{
                label: folders.length > 1 ? ( localize(9840, "&&Copy Folders")) : ( localize(9841, "&&Copy Folder")),
            ...
            if (( folders.some(folder => workspaceFolderSchemas.indexOf(folder.uri.scheme) >= 0))) {
                buttons.unshift({
                    label: folders.length > 1 ? ( localize(9842, "&&Add Folders to Workspace")) : ( localize(9843, "&&Add Folder to Workspace")),
                ...
                message = folders.length > 1 ? ( localize(9846, "Are you sure to want to copy folders?")) : ( localize(9847, "Are you sure to want to copy '{0}'?", basename(folders[0].uri)));
            }
            const {
                result
    ...
    async importResources(target, resources, token) {
        if (resources && resources.length > 0) {
            ...
            if (inaccessibleFileCount > 0) {
                this.notificationService.error(inaccessibleFileCount > 1 ? ( localize(9848, "Some or all of the dropped files could not be accessed for import.")) : ( localize(9849, "The dropped file could not be accessed for import.")));
            }
            ...
            await this.explorerService.applyBulkEdit(resourceFileEdits, {
                undoLabel: resourcesFiltered.length === 1 ? ( localize(9850, "Import {0}", basename(resourcesFiltered[0]))) : ( localize(9851, "Import {0} resources", resourcesFiltered.length)),
                progressLabel: resourcesFiltered.length === 1 ? ( localize(9852, "Copying {0}", basename(resourcesFiltered[0]))) : ( localize(9853, "Copying {0} resources", resourcesFiltered.length)),
...
let FileDownload = class FileDownload {
    ...
    download(source) {
        ...
        const downloadPromise = this.progressService.withProgress({
            ...
            delay: 800,
            cancellable: isWeb,
            title: ( localize(9854, "Downloading"))
    ...
    async doDownloadNative(explorerItem, progress, cts) {
        ...
        const destination = await this.fileDialogService.showSaveDialog({
            ...
            saveLabel: ( localize(9857, "Download")),
            title: ( localize(9858, "Choose Where to Download")),
            defaultUri
        });
        if (destination) {
            ...
            await this.explorerService.applyBulkEdit([( new ResourceFileEdit(explorerItem.resource, destination, {
                ...
                undoLabel: ( localize(9859, "Download {0}", explorerItem.name)),
                progressLabel: ( localize(9860, "Downloading {0}", explorerItem.name)),
                progressLocation: ProgressLocation.Window
            });
        } else {
            cts.cancel();
...
function getFileOverwriteConfirm(name) {
    return {
        ...
        detail: ( localize(9862, "This action is irreversible!")),
        primaryButton: ( localize(9863, "&&Replace")),
        type: "warning"
    };
}
function getMultipleFilesOverwriteConfirm(files) {
    if (files.length > 1) {
        return {
            ...
            detail: getFileNamesMessage(files) + "\n" + ( localize(9862, "This action is irreversible!")),
            primaryButton: ( localize(9863, "&&Replace")),
            type: "warning"
        };
    }
