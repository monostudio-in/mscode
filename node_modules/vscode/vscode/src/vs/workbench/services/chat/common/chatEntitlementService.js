...
function isProUser(chatEntitlement) {
    return chatEntitlement === ChatEntitlement.EDU || chatEntitlement === ChatEntitlement.Pro || chatEntitlement === ChatEntitlement.ProPlus || chatEntitlement === ChatEntitlement.Business || chatEntitlement === ChatEntitlement.Enterprise;
}
function getChatPlanName(chatEntitlement) {
    switch (chatEntitlement) {
    case ChatEntitlement.EDU:
        return localize(16044, "Copilot EDU");
    case ChatEntitlement.Pro:
        return localize(16045, "Copilot Pro");
    case ChatEntitlement.ProPlus:
        return localize(16046, "Copilot Pro+");
    case ChatEntitlement.Business:
        return localize(16047, "Copilot Business");
    case ChatEntitlement.Enterprise:
        return localize(16048, "Copilot Enterprise");
    default:
        return localize(16049, "Copilot Free");
    }
}
...
ChatEntitlementService = ( __decorate([( __param(0, IInstantiationService)), ( __param(1, IProductService)), ( __param(2, IWorkbenchEnvironmentService)), ( __param(3, IContextKeyService)), ( __param(4, IConfigurationService)), ( __param(5, ITelemetryService))], ChatEntitlementService));
let ChatEntitlementRequests = class ChatEntitlementRequests extends Disposable {
    ...
    async doSignUpFree(sessions) {
        ...
        if (!response) {
            const retry = await this.onUnknownSignUpError(( localize(16050, "No response received.")), "[chat entitlement] sign-up: no response");
        ...
        if (response.res.statusCode && response.res.statusCode !== 200) {
            ...
            const retry = await this.onUnknownSignUpError(( localize(16051, "Unexpected status code {0}.", response.res.statusCode)), `[chat entitlement] sign-up: unexpected status code ${response.res.statusCode}`);
            return retry ? this.doSignUpFree(sessions) : {
                errorCode: response.res.statusCode
            };
        }
        ...
        if (!responseText) {
            const retry = await this.onUnknownSignUpError(( localize(16052, "Response has no contents.")), "[chat entitlement] sign-up: response has no content");
            return retry ? this.doSignUpFree(sessions) : {
                errorCode: 2
            };
        }
        ...
        try {
            ...
            const retry = await this.onUnknownSignUpError(( localize(16053, "Invalid response contents.")), `[chat entitlement] sign-up: error parsing response (${err})`);
            return retry ? this.doSignUpFree(sessions) : {
                errorCode: 3
            };
        }
    ...
    async onUnknownSignUpError(detail, logMessage) {
        ...
        if (!this.lifecycleService.willShutdown) {
            const {
                ...
                detail,
                primaryButton: ( localize(16055, "Retry"))
            });
            return confirmed;
        }
        return false;
    ...
    onUnprocessableSignUpError(logMessage, logDetails) {
        ...
        if (!this.lifecycleService.willShutdown) {
            this.dialogService.prompt({
                ...
                detail: logDetails,
                buttons: [{
                    label: ( localize(16057, "OK")),
                    run: () => {}
                }, {
                    label: ( localize(16058, "Learn More")),
...
export { ChatEntitlement, ChatEntitlementContext, ChatEntitlementContextKeys, ChatEntitlementRequests, ChatEntitlementService, getChatPlanName, isProUser };
