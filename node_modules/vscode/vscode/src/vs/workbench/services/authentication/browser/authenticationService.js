...
const authenticationDefinitionSchema = {
    ...
    properties: {
        id: {
            type: "string",
            description: ( localize(16031, "The id of the authentication provider."))
        },
        label: {
            type: "string",
            description: ( localize(16032, "The human readable name of the authentication provider."))
        },
...
const authenticationExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(16035, "Contributes authentication")),
        type: "array",
...
let AuthenticationService = class AuthenticationService extends Disposable {
            ...
            added.forEach(point => {
                for (const provider of point.value) {
                    if (isFalsyOrWhitespace(provider.id)) {
                        point.collector.error(( localize(16036, "An authentication contribution must specify an id.")));
                        continue;
                    }
                    if (isFalsyOrWhitespace(provider.label)) {
                        point.collector.error(( localize(16037, "An authentication contribution must specify a label.")));
                        continue;
                    }
    ...
    registerDeclaredAuthenticationProvider(provider) {
        if (isFalsyOrWhitespace(provider.id)) {
            throw ( new Error(( localize(16036, "An authentication contribution must specify an id."))));
        }
        if (isFalsyOrWhitespace(provider.label)) {
            throw ( new Error(( localize(16037, "An authentication contribution must specify a label."))));
        }
        if (( this.declaredProviders.some(p => p.id === provider.id))) {
    ...
    async createDynamicAuthenticationProvider(authorizationServer, serverMetadata, resource, clientId) {
    ...
    async tryActivateProvider(providerId, activateImmediate) {
        ...
        try {
            const result = await raceTimeout(raceCancellation(Event.toPromise(
                Event.filter(this.onDidRegisterAuthenticationProvider, e => e.id === providerId, store),
                store
            ), this._disposedSource.token), 5000);
