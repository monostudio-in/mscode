...
let ExtensionsSynchroniser = class ExtensionsSynchroniser extends AbstractSynchroniser {
    constructor(
        ...
        ), Event.filter(this.extensionManagementService.onDidUninstallExtension, (e => !e.error)), Event.filter(userDataProfileStorageService.onDidChange, e => ( e.valueChanges.some((
            {
                profile,
                changes
            }
        ) => this.syncResource.profile.id === profile.id && ( changes.some(change => change.key === DISABLED_EXTENSIONS_STORAGE_PATH))))), extensionStorageService.onDidChangeExtensionStorageToSync)(() => this.triggerLocalChange()));
    }
                ...
                await Promises.settled(( [...added, ...updated].map(async e => {
                    ...
                    if (extension) {
                        try {
                            ...
                            if (
                            !installedExtension || installedExtension.preRelease !== e.preRelease || installedExtension.pinned !== e.pinned || (version && installedExtension.manifest.version !== version)) {
