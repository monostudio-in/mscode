...
import { $ as $$1, append, prepend, isAncestorOfActiveElement } from '../../../../base/browser/dom.js';
...
let QuickInputTreeRenderer = class QuickInputTreeRenderer extends Disposable {
    ...
    renderTemplate(container) {
        ...
        const checkbox = store.add(( new TriStateCheckbox("", false, {
    ...
    renderElement(node, _index, templateData, _details) {
        ...
        if (quickTreeItem.pickable === false) {
            ...
            store.add(
                Event.filter(this.onCheckedEvent, e => e.item === quickTreeItem)(e => checkbox.checked = e.checked)
            );
            if (quickTreeItem.disabled) {
                checkbox.disable();
            }
            store.add(checkbox.onChange(
                e => this._checkboxStateHandler.setCheckboxState(quickTreeItem, checkbox.checked)
            ));
        }
        ...
        if (buttons && buttons.length) {
            const {
                primary,
                secondary
            } = quickInputButtonsToActionArrays(buttons, "quick-input-tree", button => this._buttonTriggeredEmitter.fire({
                item: quickTreeItem,
                button
            }));
...
QuickInputTreeRenderer = QuickInputTreeRenderer_1 = ( __decorate([( __param(5, IContextMenuService)), ( __param(6, IThemeService))], QuickInputTreeRenderer));
