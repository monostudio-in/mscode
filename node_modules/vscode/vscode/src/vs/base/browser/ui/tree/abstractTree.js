...
class FindToggles {
    constructor(startStates) {
        this.stateMap = ( new Map(( startStates.map(state => [state.id, {
            ...state
        }]))));
    }
    states() {
...
class FindWidget extends Disposable {
    ...
    constructor(
        ...
        this.findInput = this._register(( new FindInput(this.elements.findInput, contextViewProvider, {
            label: ( localize(34, "Type to search")),
            placeholder,
        ...
        const closeAction = this._register(( new Action("close", ( localize(35, "Close")), "codicon codicon-close", true, () => this.dispose())));
...
class AbstractFindController {
    ...
    constructor(tree, filter, contextViewProvider, options = {}) {
        ...
        this._placeholder = options.placeholder ?? ( localize(34, "Type to search"));
    }
    isOpened() {
        return !!this.widget;
    }
        ...
        if (showNotFound) {
            if (this.tree.options.showNotFoundMessage ?? true) {
                this.widget?.showMessage({
                    ...
                    content: warningMessage ?? ( localize(36, "No results found."))
                });
            } else {
                this.widget?.showMessage({
                    type: MessageType.WARNING
...
class FindController extends AbstractFindController {
    ...
    set mode(mode) {
        ...
        this.placeholder = isFilterMode ? ( localize(39, "Type to filter")) : ( localize(34, "Type to search"));
...
class StickyScrollWidget {
    ...
    setAccessibilityAttributes(container, element, stickyIndex, stickyNodesTotal) {
        ...
        if (this.accessibilityProvider.getSetSize) {
            container.setAttribute("aria-setsize", String(
                this.accessibilityProvider.getSetSize(element, stickyIndex, stickyNodesTotal)
            ));
        }
...
class AbstractTree {
    ...
    setupModel(model) {
        ...
        this.modelDisposables.add(model.onDidSpliceRenderedNodes((
            {
                start,
                deleteCount,
                elements
