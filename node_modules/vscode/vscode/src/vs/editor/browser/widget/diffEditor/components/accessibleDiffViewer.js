...
const accessibleDiffViewerInsertIcon = registerIcon("diff-review-insert", Codicon.add, ( localize(179, "Icon for 'Insert' in accessible diff viewer.")));
const accessibleDiffViewerRemoveIcon = registerIcon("diff-review-remove", Codicon.remove, ( localize(180, "Icon for 'Remove' in accessible diff viewer.")));
const accessibleDiffViewerCloseIcon = registerIcon("diff-review-close", Codicon.close, ( localize(181, "Icon for 'Close' in accessible diff viewer.")));
...
let View = class View extends Disposable {
    ...
    _render(store) {
        ...
        container.setAttribute("aria-label", ( localize(183, "Accessible Diff Viewer. Use arrow up and down to navigate.")));
        ...
        for (const viewItem of group?.lines || []) {
            ...
            if (viewItem.type === LineType.Header) {
                ...
                const getAriaLines = lines => lines === 0 ? ( localize(184, "no lines changed")) : lines === 1 ? ( localize(185, "1 line changed")) : ( localize(186, "{0} lines changed", lines));
    ...
    _createRow(
        ...
        if (lineContent.length === 0) {
            lineContent = ( localize(188, "blank"));
        }
        let ariaLabel = "";
        switch (item.type) {
        case LineType.Unchanged:
            if (item.originalLineNumber === item.modifiedLineNumber) {
                ariaLabel = ( localize(189, "{0} unchanged line {1}", lineContent, item.originalLineNumber));
            } else {
                ariaLabel = ( localize(
                    190,
            ...
            break;
        case LineType.Added:
            ariaLabel = ( localize(191, "+ {0} modified line {1}", lineContent, item.modifiedLineNumber));
            break;
        case LineType.Deleted:
            ariaLabel = ( localize(192, "- {0} original line {1}", lineContent, item.originalLineNumber));
            break;
        }
