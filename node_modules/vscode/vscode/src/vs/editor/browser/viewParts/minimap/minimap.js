...
class InnerMinimap extends Disposable {
    constructor(theme, model) {
        ...
        this._register(
            addStandardDisposableListener(this._domNode.domNode, EventType.MOUSE_OVER, () => {
                this._isMouseOverMinimap = true;
            })
        );
        this._register(
            addStandardDisposableListener(this._domNode.domNode, EventType.MOUSE_LEAVE, () => {
                this._isMouseOverMinimap = false;
            })
        );
    ...
    getXOffsetForPosition(lineOffsetMap, lineNumber, column, tabSize, charWidth, canvasInnerWidth) {
        ...
        if (!lineIndexToXOffset) {
            ...
            for (let i = 1; i < lineData.length + 1; i++) {
                ...
                const dx = charCode === CharCode.Tab ? tabSize * charWidth : isFullWidthCharacter(charCode) ? 2 * charWidth : charWidth;
                const x = prevx + dx;
