...
let InlineEditsView = class InlineEditsView extends Disposable {
    constructor(
        ...
        this._uiState = derived(this, reader => {
            ...
            if (this._showCollapsed.read(reader)) {
                state = {
                    kind: InlineCompletionViewKind.Collapsed,
        ...
        this._previewTextModel = this._register(
            this._instantiationService.createInstance(TextModel, "", this._editor.getModel().getLanguageId(), {
        ...
        this._sideBySide = this._register(
            this._instantiationService.createInstance(InlineEditsSideBySideView, this._editor, ( this._model.map(m => m?.inlineEdit)), this._previewTextModel, ( this._uiState.map(s => s && s.state?.kind === InlineCompletionViewKind.SideBySide ? ({
                newTextLineCount: s.newTextLineCount,
                editorType: s.editorType
            }) : undefined)), this._tabAction)
        );
        this._deletion = this._register(
            this._instantiationService.createInstance(InlineEditsDeletionView, this._editor, ( this._model.map(m => m?.inlineEdit)), ( this._uiState.map(s => s && s.state?.kind === InlineCompletionViewKind.Deletion ? ({
        ...
        this._inlineCollapsedView = this._register(
            this._instantiationService.createInstance(InlineEditsCollapsedView, this._editor, ( this._model.map(
        ...
        this._longDistanceHint = derived(this, reader => {
            ...
            return reader.store.add(
                this._instantiationService.createInstance(InlineEditsLongDistanceHint, this._editor, ( this._uiState.map((s, reader) => s?.longDistanceHint ? ({
        ...
        this._jumpToView = this._register(this._instantiationService.createInstance(JumpToView, this._editorObs, {
            ...
            if (s?.state?.kind === InlineCompletionViewKind.JumpTo) {
                return {
                    jumpToPosition: s.state.position
                };
            }
            return undefined;
        })));
        ...
        this._wordReplacementViews = mapObservableArrayCached(this, wordReplacements, (viewData, store) => {
            return store.add(
                this._instantiationService.createInstance(InlineEditsWordReplacementView, this._editorObs, viewData, this._tabAction)
            );
        });
        this._lineReplacementView = this._register(
            this._instantiationService.createInstance(InlineEditsLineReplacementView, this._editorObs, ( this._uiState.map(s => s?.state?.kind === InlineCompletionViewKind.LineReplacement ? ({
                originalRange: s.state.originalRange,
        ...
        if (model.editorType !== InlineCompletionEditorType.DiffEditor) {
            ...
            if (allInnerChangesNotTooLong && isSingleInnerEdit && numOriginalLines === 1 && numModifiedLines === 1) {
                ...
                if (!( modifiedText.some(v => v.includes("\t"))) && !( originalText.some(v => v.includes("\t")))) {
                    if (!( inner.some(m => m.originalRange.isEmpty())) || !( growEditsUntilWhitespace(( inner.map(m => ( new TextReplacement(m.originalRange, "")))), inlineEdit.originalText).some(
    ...
    _determineRenderState(model, reader, diff, newText) {
        ...
        case InlineCompletionViewKind.InsertionInline:
            return {
                kind: InlineCompletionViewKind.InsertionInline,
                viewData
            };
        case InlineCompletionViewKind.SideBySide:
            return {
                kind: InlineCompletionViewKind.SideBySide,
                viewData
            };
        ...
        case InlineCompletionViewKind.Custom:
            return {
                kind: InlineCompletionViewKind.Custom,
...
InlineEditsView = ( __decorate([( __param(5, IInstantiationService))], InlineEditsView));
