...
let GhostTextView = class GhostTextView extends Disposable {
    constructor(_editor, _data, options, _languageService) {
        ...
        this._additionalLinesWidget = this._register(( new AdditionalLinesWidget(this._editor, derivedOpts({
            owner: this,
            equalsFn: equalsIfDefinedC(thisEqualsC())
        }, reader => {
            const uiState = this._state.read(reader);
...
GhostTextView = ( __decorate([( __param(3, ILanguageService))], GhostTextView));
