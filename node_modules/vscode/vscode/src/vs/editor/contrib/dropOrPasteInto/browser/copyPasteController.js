...
const pasteWidgetVisibleCtx = ( new RawContextKey("pasteWidgetVisible", false, ( localize(1047, "Whether the paste widget is showing"))));
...
let CopyPasteController = class CopyPasteController extends Disposable {
    ...
    constructor(
        ...
        this._postPasteWidgetManager = this._register(
            instantiationService.createInstance(PostEditWidgetManager, "pasteIntoEditor", editor, pasteWidgetVisibleCtx, {
                ...
                label: ( localize(1048, "Show paste options..."))
    ...
    showPasteAsNoEditMessage(selections, preference) {
        const kindLabel = "only" in preference ? preference.only.value : "preferences" in preference ? (preference.preferences.length ? ( preference.preferences.map(preference => preference.value)).join(", ") : ( localize(1049, "empty"))) : preference.providerId;
        MessageController.get(this._editor)?.showMessage(( localize(1050, "No paste edits for '{0}' found", kindLabel)), selections[0].getStartPosition());
    }
    doPasteInline(allProviders, selections, dataTransfer, metadata, clipboardEvent) {
            ...
            try {
                ...
                if (editSession.edits.length) {
                    ...
                    return this._postPasteWidgetManager.applyEditAndShowIfNeeded(selections, {
                        ...
                        const resolved = await this._pasteProgressManager.showWhile(selections[0].getEndPosition(), ( localize(1051, "Resolving paste edit for '{0}'. Click to cancel", edit.title)), raceCancellation(Promise.race([showP.p, resolveP]), resolveToken), {
                            cancel: () => showP.cancel()
                        }, 0);
                        if (resolved) {
        ...
        this._pasteProgressManager.showWhile(selections[0].getEndPosition(), ( localize(1052, "Running paste handlers. Click to cancel and do basic paste")), p, {
            cancel: async () => {
                p.cancel();
    ...
    showPasteAsPick(preference, allProviders, selections, dataTransfer, metadata) {
            ...
            try {
                ...
                if (preference) {
                    ...
                    const configureDefaultItem = {
                        ...
                        label: ( localize(1053, "Configure default paste action")),
                        edit: undefined
                    };
                    const selected = await this._quickInputService.pick([...( editSession.edits.map(edit => ({
                        ...
                        placeHolder: ( localize(1054, "Select Paste Action"))
                    });
        ...
        this._progressService.withProgress({
            ...
            title: ( localize(1055, "Running paste handlers"))
        }, () => p);
    }
