...
let InlineCompletionResults = class InlineCompletionResults extends RefCountedDisposable {
    ...
    canBeReused(model, line, word) {
        return (
            this.model === model && this.line === line && this.word.word.length > 0 && this.word.startColumn === word.startColumn && this.word.endColumn < word.endColumn && this.completionModel.getIncompleteProvider().size === 0
        );
    }
    get items() {
        const result = [];
...
let SuggestInlineCompletions = class SuggestInlineCompletions extends Disposable {
    ...
    async provideInlineCompletions(model, position, context, token) {
        ...
        if (!triggerCharacterInfo && this._lastResult?.canBeReused(model, position.lineNumber, wordInfo)) {
            ...
            const completionModel = ( new CompletionModel(completions.items, position.column, ( new LineContext(leadingLineContents, 0)), WordDistance.None, editor.getOption(EditorOption.suggest), editor.getOption(EditorOption.snippetSuggestions), {
