...
class ParameterHintsModel extends Disposable {
    ...
    async doTrigger(triggerId) {
        ...
        this.state = new ParameterHintState.Pending(createCancelablePromise(
            token => provideSignatureHelp(this.providers, model, position, triggerContext, token)
        ), activeSignatureHelp);
        try {
    ...
    get isTriggered() {
        return this.state.type === ParameterHintState.Type.Active || this.state.type === ParameterHintState.Type.Pending || this.throttledDelayer.isTriggered();
    }
    onModelChanged() {
        this.cancel();
