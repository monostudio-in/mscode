...
class AbstractGotoLineQuickAccessProvider extends AbstractEditorNavigationQuickAccessProvider {
    ...
    provideWithoutTextEditor(picker) {
        const label = ( localize(1470, "Open a text editor first to go to a line or an offset."));
        picker.items = [{
            label
        }];
    ...
    provideWithTextEditor(context, picker, token) {
        ...
        const offsetButton = {
            ...
            tooltip: ( localize(1472, "Toggle Zero-Based Offset")),
    ...
    parsePosition(editor, value) {
        ...
        if (!model) {
            return {
                label: ( localize(1470, "Open a text editor first to go to a line or an offset."))
            };
        }
        if (value.startsWith(":")) {
            ...
            if (isNaN(offset)) {
                return {
                    inOffsetMode: true,
                    label: this.useZeroBasedOffset ? ( localize(1474, "Type a character position to go to (from 0 to {0}).", maxOffset - 1)) : ( localize(1475, "Type a character position to go to (from 1 to {0}).", maxOffset))
                };
            } else {
            ...
            if (parts.length < 1 || isNaN(lineNumber)) {
                return {
                    label: ( localize(1477, "Type a line number to go to (from 1 to {0}).", maxLine))
                };
            }
